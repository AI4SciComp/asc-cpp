...
struct Refinement {
  ...
  Refinement(const std::array<T, 4>& matrix, asc::DenseBlasLayout b_format,
    ...
    for (int i = 0; i < 2; ++i) {
      for (int j = 0; j < 2; ++j) {
        ...
        x[Offset(i, j, x_layout)] =
            Exact(i, j, pass) + Value<T>(0.0625, -0.03125);
      }
    }
  }
...
template <typename T>
bool RefineTwice(const asc::ReferenceLapackProvider& provider,
  ...
  Refinement<T> data(matrix, b_layout, x_layout);
  const auto original =
  ...
  for (int pass = 0; pass < 2; ++pass) {
    ...
    if (!asc::Ptrfs(provider, original, factor, rhs, solution, ferr, berr, plan,
                    workspace, report)
             .ok() ||
        !report.called_provider || report.native_info != 0 ||
        report.output_validity != asc::LapackOutputValidity::kComplete ||
        report.factor_family.has_value() || !data.Check(pass) ||
        data.b != b_before) {
      return false;
    }
  }
...
bool Reuse(const asc::ReferenceLapackProvider& provider,
  ...
  for (const auto b :
    ...
    for (const auto x : {asc::DenseBlasLayout::kColumnMajor,
      ...
      if (!RefineTwice(provider, factor, matrix, b, x)) {
        return false;
      }
    }
  }
...
bool Run(const asc::ReferenceLapackProvider& provider) {
  ...
  // once, then reused by refinement with known independent right-hand sides.
  ...
  std::printf(
      "PTRFS scalar_bytes=%zu: known solutions and finite estimates, borrowed "
