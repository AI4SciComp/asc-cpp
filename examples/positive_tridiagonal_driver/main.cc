...
bool Run(const asc::ReferenceLapackProvider& provider,
  ...
  b.fill(Value<T>(-77));
  const auto rhs = Take(asc::DenseBlasMatrixView<T>::Create(
      b.data(), 3, 1, layout, layout == asc::DenseBlasLayout::kRowMajor ? 2 : 4,
      {b.data(), sizeof(b), kHost}));
  const auto at = [layout](std::size_t i) {
    return layout == asc::DenseBlasLayout::kRowMajor ? 2 * i : i;
  };
  for (std::size_t i = 0; i < 3; ++i) {
    b[at(i)] = original[3 * i] + original[3 * i + 1] + original[3 * i + 2];
  }
  const auto plan = Take(asc::QueryPtsvWorkspace(provider, matrix, rhs));
  asc::LapackWorkspace workspace;
  constexpr auto kPacking =
      static_cast<std::size_t>(asc::LapackWorkspaceKind::kLayoutConversion);
  if (plan.regions[kPacking].minimum_entries != 3) {
    return false;
  }
  workspace.regions[kPacking] = {staging.data(), sizeof(staging), kHost};
  asc::LapackReport report;
  if (!asc::Ptsv(provider, matrix, rhs, plan, workspace, report).ok() ||
      !report.called_provider || report.native_info != 0 ||
      report.output_validity != asc::LapackOutputValidity::kComplete) {
    return false;
  }
  for (std::size_t i = 0; i < 3; ++i) {
    if (std::abs(b[at(i)] - T{1}) > 64 * std::numeric_limits<Real>::epsilon()) {
      return false;
    }
  }
  const auto factor =
  ...
  std::printf(
      "PTSV public example real_bytes=%zu scalar_bytes=%zu: factor once, "
      "borrowed "
...
int main() {
  ...
  for (const auto layout :
    ...
    if (!Run<float>(provider, layout) || !Run<double>(provider, layout) ||
        !Run<std::complex<float>>(provider, layout) ||
        !Run<std::complex<double>>(provider, layout)) {
      return 1;
    }
  }
  return 0;
