#ifndef ASC_DENSE_PROVIDERS_LAPACK_INDEFINITE_CONDITION_H_
#define ASC_DENSE_PROVIDERS_LAPACK_INDEFINITE_CONDITION_H_

/** @file
 * @brief Explicit classic block-factor reciprocal-condition estimation.
 *
 * SYCON consumes real/complex symmetric factors using transpose; HECON
 * consumes complex Hermitian factors using conjugate transpose. RCOND is an
 * estimate of the reciprocal one-norm condition number, not an accuracy
 * certificate. Complex one-norms use Euclidean scalar modulus. The caller
 * supplies a finite nonnegative norm of the original matrix and matching
 * same-provider classic Bunch–Kaufman factor/pivot storage. Raw descriptors
 * cannot prove historical provenance or certify a successful factorization.
 *
 ...
 * one-based pivots are checked with classic upper/lower directional bounds;
 * rook/Aasen/other families are rejected, never normalized into permutations.
 *
 ...
 * Any nonzero foreign INFO is a provider defect, retained verbatim with
...
/** @brief Queries single real SYCON without a foreign call or array reads.
 * @param provider Explicit checked reference selection; unchanged.
 * @param triangle Selected upper/lower factor triangle.
 * @param factors Immutable square classic symmetric factor; entries unread.
 ...
 */
ASC_DENSE_LAPACK_EXPORT Result<LapackWorkspacePlan> QuerySyconWorkspace(
...
Sycon(const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
      DenseBlasMatrixView<const float> factors, RawLapackPivotView pivots,
      float original_norm, float& reciprocal_condition,
      const LapackWorkspacePlan& plan, const LapackWorkspace& workspace,
      LapackReport& report);

/** @brief Queries double real SYCON without a foreign call or array reads.
 * @param provider Explicit checked reference selection; unchanged.
 * @param triangle Selected upper/lower factor triangle.
 * @param factors Immutable square classic symmetric factor; entries unread.
 * @param pivots Matching raw classic block pivots, exact n; entries unread.
...
/** @brief Queries single complex SYCON using transpose-based symmetry.
 * @param provider Explicit checked reference selection; unchanged.
 * @param triangle Selected upper/lower factor triangle.
...
/** @brief Queries double complex SYCON using transpose-based symmetry.
 * @param provider Explicit checked reference selection; unchanged.
 * @param triangle Selected upper/lower factor triangle.
 * @param factors Immutable square complex symmetric factor; entries unread.
 * @param pivots Matching raw classic block pivots, exact n; entries unread.
...
/** @brief Queries single complex HECON using Hermitian symmetry.
 * @param provider Explicit checked reference selection; unchanged.
 * @param triangle Selected upper/lower factor triangle.
 * @param factors Immutable square classic Hermitian factor; entries unread.
 * @param pivots Matching raw classic block pivots, exact n; entries unread.
 * @param original_norm Finite nonnegative original-matrix one-norm.
 * @param reciprocal_condition Disjoint live real host output; unread.
 * @return Complete formula plan or structural error; no foreign call/writes.
 */
ASC_DENSE_LAPACK_EXPORT Result<LapackWorkspacePlan> QueryHeconWorkspace(
    const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
    DenseBlasMatrixView<const std::complex<float>> factors,
    RawLapackPivotView pivots, float original_norm,
    const float& reciprocal_condition);
 ...
 * @param provider Explicit same-build reference provider; no fallback.
 * @param triangle Selected upper/lower factor triangle.
 * @param factors Immutable square classic Hermitian factor, full coefficients.
 * @param pivots Matching raw classic signed paired pivots, exact n.
 * @param original_norm Finite nonnegative original-matrix one-norm.
 * @param reciprocal_condition Disjoint live real host RCOND output.
 * @param plan Unmodified matching formula query result.
 ...
 */
ASC_DENSE_LAPACK_EXPORT Status
Hecon(const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
      DenseBlasMatrixView<const std::complex<float>> factors,
      RawLapackPivotView pivots, float original_norm,
      float& reciprocal_condition, const LapackWorkspacePlan& plan,
      const LapackWorkspace& workspace, LapackReport& report);

/** @brief Queries double complex HECON using Hermitian symmetry.
 * @param provider Explicit checked reference selection; unchanged.
 * @param triangle Selected upper/lower factor triangle.
 * @param factors Immutable square classic Hermitian factor; entries unread.
 * @param pivots Matching raw classic block pivots, exact n; entries unread.
...
#endif  // ASC_DENSE_PROVIDERS_LAPACK_INDEFINITE_CONDITION_H_
