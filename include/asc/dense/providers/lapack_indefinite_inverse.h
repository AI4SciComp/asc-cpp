#ifndef ASC_DENSE_PROVIDERS_LAPACK_INDEFINITE_INVERSE_H_
#define ASC_DENSE_PROVIDERS_LAPACK_INDEFINITE_INVERSE_H_

/** @file
 * @brief Explicit in-place classic symmetric and Hermitian inversion.
 *
 * SYTRI inverts transpose-symmetric factors, including complex symmetric
 * factors. HETRI inverts conjugate-transpose Hermitian factors. The caller
 * supplies mutable selected square factor storage and immutable raw
 * kBunchKaufman pivots from one same-provider, same-scalar SYTRF/SYTF2 or
 * HETRF/HETF2 operation (including the corresponding direct driver).
 * Adjacent equal negative entries encode one ordered interchange for a
 ...
 * materialized.
 *
 * Active calls require n live T scalar WORK entries for S/D SYTRI and C/Z
 * HETRI, or 2*n entries for C/Z SYTRI as documented by the pinned LAPACK
 * contract, and n provider-width signed INTEGER entries in caller byte storage.
 * Row-major A additionally needs n*n live T layout entries. The formula query
 * inspects metadata only, makes no foreign call, and does not read factor or
 * pivot entries. Plans bind routine, scalar, provider/ABI, triangle, symmetry,
 * order, layout and original/effective leading dimensions. N=0 needs no
 * workspace or numerical reads/writes and succeeds without a foreign call.
 * Source loop and BLAS cursor arithmetic is checked against the selected
 * INTEGER width before numerical access.
 *
...
/** @brief Queries single real SSYTRI workspace without array reads.
 ...
 */
ASC_DENSE_LAPACK_EXPORT Result<LapackWorkspacePlan> QuerySytriWorkspace(
    const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
    DenseBlasMatrixView<float> factors, RawLapackPivotView pivots);

/** @brief Executes native single real SSYTRI in place.
 ...
 * promise.
 */
ASC_DENSE_LAPACK_EXPORT Status Sytri(const ReferenceLapackProvider& provider,
...
/** @brief Queries double real DSYTRI workspace without array reads.
 * @param provider Explicit checked provider; unchanged.
 * @param triangle Selected upper/lower factor and inverse triangle.
 * @param factors Mutable square raw factor descriptor; values unread here.
 * @param pivots Same-operation immutable signed kBunchKaufman pivots, exact n.
...
/** @brief Executes native double real DSYTRI in place.
 * @param provider Explicit same-build provider; no fallback.
 * @param triangle Same-call selected upper/lower triangle.
 * @param factors Raw factor, replaced by selected inverse on native success.
 * @param pivots Immutable same-factor classic block pivots; never
...
/** @brief Queries single complex CSYTRI workspace without array reads.
 * @param provider Explicit checked provider; unchanged.
 * @param triangle Selected upper/lower factor and inverse triangle.
 * @param factors Mutable square raw factor descriptor; values unread here.
 * @param pivots Same-operation immutable signed kBunchKaufman pivots, exact n.
...
/** @brief Executes native single complex CSYTRI in place.
 * @param provider Explicit same-build provider; no fallback.
 * @param triangle Same-call selected upper/lower triangle.
 * @param factors Raw factor, replaced by selected inverse on native success.
 * @param pivots Immutable same-factor classic block pivots; never
...
Sytri(const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
      DenseBlasMatrixView<std::complex<float>> factors,
      RawLapackPivotView pivots, const LapackWorkspacePlan& plan,
      const LapackWorkspace& workspace, LapackReport& report);

/** @brief Queries double complex ZSYTRI workspace without array reads.
 * @param provider Explicit checked provider; unchanged.
 * @param triangle Selected upper/lower factor and inverse triangle.
 * @param factors Mutable square raw factor descriptor; values unread here.
 * @param pivots Same-operation immutable signed kBunchKaufman pivots, exact n.
...
/** @brief Executes native double complex ZSYTRI in place.
 * @param provider Explicit same-build provider; no fallback.
 * @param triangle Same-call selected upper/lower triangle.
 * @param factors Raw factor, replaced by selected inverse on native success.
 * @param pivots Immutable same-factor classic block pivots; never
...
/** @brief Queries single complex CHETRI workspace without array reads.
 * @param provider Explicit checked provider; unchanged.
 * @param triangle Selected upper/lower factor and inverse triangle.
 * @param factors Mutable square raw factor descriptor; values unread here.
 * @param pivots Same-operation immutable signed kBunchKaufman pivots, exact n.
 * @return Formula plan or structural/placement/alias/arithmetic failure.
 */
ASC_DENSE_LAPACK_EXPORT Result<LapackWorkspacePlan> QueryHetriWorkspace(
    const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
    DenseBlasMatrixView<std::complex<float>> factors,
    RawLapackPivotView pivots);

/** @brief Executes native single complex CHETRI in place.
 * @param provider Explicit same-build provider; no fallback.
 * @param triangle Same-call selected upper/lower triangle.
 * @param factors Raw factor, replaced by selected inverse on native success.
 * @param pivots Immutable same-factor classic block pivots; never
 ...
 */
ASC_DENSE_LAPACK_EXPORT Status
Hetri(const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
      DenseBlasMatrixView<std::complex<float>> factors,
      RawLapackPivotView pivots, const LapackWorkspacePlan& plan,
      const LapackWorkspace& workspace, LapackReport& report);

/** @brief Queries double complex ZHETRI workspace without array reads.
 * @param provider Explicit checked provider; unchanged.
 * @param triangle Selected upper/lower factor and inverse triangle.
 * @param factors Mutable square raw factor descriptor; values unread here.
 * @param pivots Same-operation immutable signed kBunchKaufman pivots, exact n.
...
/** @brief Executes native double complex ZHETRI in place.
 * @param provider Explicit same-build provider; no fallback.
 * @param triangle Same-call selected upper/lower triangle.
 * @param factors Raw factor, replaced by selected inverse on native success.
 * @param pivots Immutable same-factor classic block pivots; never
...
#endif  // ASC_DENSE_PROVIDERS_LAPACK_INDEFINITE_INVERSE_H_
