#ifndef ASC_DENSE_PROVIDERS_LAPACK_LU_BAND_REFINEMENT_H_
#define ASC_DENSE_PROVIDERS_LAPACK_LU_BAND_REFINEMENT_H_

/** @file
 * @brief Explicit reference general-system iterative refinement and estimates.
 *
 * GBRFS refines caller-owned X for op(A)*X=B, preserving original A/B, raw LU
 * and raw signed one-based band pivots from the same factorization. The caller
 * guarantees those raw factors/pivots correspond to original A and that X is an
 * initial solution of that system. Raw descriptors do not fabricate provenance
 * or a successful-factor certificate. N/T/C are distinct legal complex modes.
 *
 ...
 * real and packing objects must already be live. Each independently row-major
 * B/X requires its rows*columns entries in caller kLayoutConversion,
 * packed consecutively in that order. Compact AB and expanded AFB are
 * column-major with all declared bandwidths retained; no densification. X alone
 * is unpacked after raw INFO=0, including the estimate-quality warnings below.
 *
 ...
 * Plans bind routine/scalar/provider, bands, all dimensions/leading dimensions,
 ...
 * INFO. All detected metadata aliases preserve the caller report; otherwise
 * report resets before preflight. For n=0 or nrhs=0, the actual pinned
 * source executes: X stays unchanged and FERR/BERR become zero. Active
 * execution rejects an exactly zero U diagonal
 ...
 * GBRFS componentwise magnitudes and FERR normalization use CABS1,
 ...
 * @param original Immutable compact column-major n-by-n AB (diagonal row KU).
 * @param factors Immutable expanded GB factors (diagonal row KL+KU), with full
 * backing.
 * @param pivots Matching nominal signed band swaps; j+1 <= p[j] <=
 * min(n,j+kl+1).
 ...
 */
ASC_DENSE_LAPACK_EXPORT Result<LapackWorkspacePlan> QueryGbrfsWorkspace(
 ...
 */
ASC_DENSE_LAPACK_EXPORT Status Gbrfs(
    const ReferenceLapackProvider& provider, DenseBlasTranspose transpose,
    ReferenceGeneralBandView<const float> original,
    LapackLuBandView<const float> factors, ReferenceLuBandPivotView pivots,
    DenseBlasMatrixView<const float> rhs, DenseBlasMatrixView<float> solution,
...
#endif  // ASC_DENSE_PROVIDERS_LAPACK_LU_BAND_REFINEMENT_H_
