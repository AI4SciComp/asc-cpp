#ifndef ASC_DENSE_PROVIDERS_LAPACK_INDEFINITE_H_
#define ASC_DENSE_PROVIDERS_LAPACK_INDEFINITE_H_

/** @file
 * @brief Explicit classic Bunch–Kaufman symmetric and Hermitian factor/solve.
 *
 * SYTRF/SYTF2 compute A=U*D*U^T or L*D*L^T, including complex symmetric A.
 * HETRF/HETF2 compute A=U*D*U^H or L*D*L^H. U/L include the documented
 * permutations; D has 1-by-1 and 2-by-2 blocks. A raw pivot vector is not a
 * permutation. Only classic Bunch–Kaufman encodings are supported here.
 *
 ...
 * SYTRF/HETRF formula queries derive minimum WORK=1 and preferred WORK from
 * the pinned ILAENV NB=64 and its actual S/C rounding. Execution uses the
 * lesser of supplied scalar capacity and preferred capacity, retaining the
 * upstream blocked/reduced-block/unblocked selection. SYTF2/HETF2/SYTRS/HETRS
 * need no scalar WORK. Empty operations need no workspace and make no foreign
 * call. Query paths inspect metadata, not matrix/pivot entries, and do not call
 * LAPACK.
 *
 ...
 * also reports a NaN pivot using positive INFO; no finiteness or conditioning
...
/** @brief Mathematical involution of a classic Bunch–Kaufman factor. */
enum class LapackBunchKaufmanSymmetry : std::uint8_t {
  kSymmetric,  ///< Real/complex symmetry uses transpose, not conjugation.
  kHermitian   ///< Complex Hermitian symmetry uses conjugate transpose.
};

/** @brief Borrows a same-provider successful classic block factor and pivots.
 *
...
class ReferenceBunchKaufmanFactorView {
 public:
   ...
   * @param pivots Same-call immutable classic signed paired pivots, exact n.
   * @param report Successful complete same-call SYTRF/SYTF2/HETRF/HETF2 report.
   ...
   */
  static ASC_DENSE_LAPACK_EXPORT Result<ReferenceBunchKaufmanFactorView> Create(
  ...
  /** @brief Returns borrowed classic signed paired pivots.
 ...
 private:
  ReferenceBunchKaufmanFactorView(DenseBlasMatrixView<const Element> factors,
...
/** @brief Queries single real SYTRF workspace without reads.
 ...
 */
ASC_DENSE_LAPACK_EXPORT Result<LapackWorkspacePlan> QuerySytrfWorkspace(
    const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
    DenseBlasMatrixView<float> matrix, DenseBlasVectorView<index_t> pivots);

/** @brief Executes actual single real SYTRF Bunch–Kaufman.
 ...
 */
ASC_DENSE_LAPACK_EXPORT Status Sytrf(const ReferenceLapackProvider& provider,
                                     DenseBlasTriangle triangle,
...
/** @brief Queries double real SYTRF workspace without reads.
 * @param provider Explicit checked reference provider; unchanged.
 * @param triangle Selected upper/lower symmetric input triangle.
 * @param matrix Square input/output descriptor; entries are not read here.
 * @param pivots Disjoint contiguous signed ASC output, exact n; entries unread.
...
/** @brief Executes actual double real SYTRF Bunch–Kaufman.
 * @param provider Explicit same-build reference selection; no fallback.
 * @param triangle Selected upper/lower symmetric input/output triangle.
 * @param matrix Square input, replaced by completed raw selected block factors.
 * @param pivots Disjoint contiguous n-entry signed one-based block output.
...
/** @brief Queries single complex SYTRF workspace without reads.
 * @param provider Explicit checked reference provider; unchanged.
 * @param triangle Selected upper/lower symmetric input triangle.
 * @param matrix Square input/output descriptor; entries are not read here.
 * @param pivots Disjoint contiguous signed ASC output, exact n; entries unread.
...
/** @brief Executes actual single complex SYTRF Bunch–Kaufman.
 * @param provider Explicit same-build reference selection; no fallback.
 * @param triangle Selected upper/lower symmetric input/output triangle.
 * @param matrix Square input, replaced by completed raw selected block factors.
 * @param pivots Disjoint contiguous n-entry signed one-based block output.
...
Sytrf(const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
      DenseBlasMatrixView<std::complex<float>> matrix,
      DenseBlasVectorView<index_t> pivots, const LapackWorkspacePlan& plan,
      const LapackWorkspace& workspace, LapackReport& report);

/** @brief Queries double complex SYTRF workspace without reads.
 * @param provider Explicit checked reference provider; unchanged.
 * @param triangle Selected upper/lower symmetric input triangle.
 * @param matrix Square input/output descriptor; entries are not read here.
 * @param pivots Disjoint contiguous signed ASC output, exact n; entries unread.
...
/** @brief Executes actual double complex SYTRF Bunch–Kaufman.
 * @param provider Explicit same-build reference selection; no fallback.
 * @param triangle Selected upper/lower symmetric input/output triangle.
 * @param matrix Square input, replaced by completed raw selected block factors.
 * @param pivots Disjoint contiguous n-entry signed one-based block output.
...
/** @brief Queries single real SYTF2 workspace without reads.
 * @param provider Explicit checked reference provider; unchanged.
 * @param triangle Selected upper/lower symmetric input triangle.
 * @param matrix Square input/output descriptor; entries are not read here.
 * @param pivots Disjoint contiguous signed ASC output, exact n; entries unread.
 * @return Complete formula-bound workspace plan or structural failure.
 */
ASC_DENSE_LAPACK_EXPORT Result<LapackWorkspacePlan> QuerySytf2Workspace(
    const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
    DenseBlasMatrixView<float> matrix, DenseBlasVectorView<index_t> pivots);

/** @brief Executes actual single real SYTF2 Bunch–Kaufman.
 * @param provider Explicit same-build reference selection; no fallback.
 * @param triangle Selected upper/lower symmetric input/output triangle.
 * @param matrix Square input, replaced by completed raw selected block factors.
 * @param pivots Disjoint contiguous n-entry signed one-based block output.
 ...
 * successful factor. Invalid native pivots/INFO are never published as valid.
 */
ASC_DENSE_LAPACK_EXPORT Status Sytf2(const ReferenceLapackProvider& provider,
                                     DenseBlasTriangle triangle,
                                     DenseBlasMatrixView<float> matrix,
                                     DenseBlasVectorView<index_t> pivots,
                                     const LapackWorkspacePlan& plan,
...
/** @brief Queries double real SYTF2 workspace without reads.
 * @param provider Explicit checked reference provider; unchanged.
 * @param triangle Selected upper/lower symmetric input triangle.
 * @param matrix Square input/output descriptor; entries are not read here.
 * @param pivots Disjoint contiguous signed ASC output, exact n; entries unread.
...
/** @brief Executes actual double real SYTF2 Bunch–Kaufman.
 * @param provider Explicit same-build reference selection; no fallback.
 * @param triangle Selected upper/lower symmetric input/output triangle.
 * @param matrix Square input, replaced by completed raw selected block factors.
 * @param pivots Disjoint contiguous n-entry signed one-based block output.
...
/** @brief Queries single complex SYTF2 workspace without reads.
 * @param provider Explicit checked reference provider; unchanged.
 * @param triangle Selected upper/lower symmetric input triangle.
 * @param matrix Square input/output descriptor; entries are not read here.
 * @param pivots Disjoint contiguous signed ASC output, exact n; entries unread.
...
/** @brief Executes actual single complex SYTF2 Bunch–Kaufman.
 * @param provider Explicit same-build reference selection; no fallback.
 * @param triangle Selected upper/lower symmetric input/output triangle.
 * @param matrix Square input, replaced by completed raw selected block factors.
 * @param pivots Disjoint contiguous n-entry signed one-based block output.
 ...
 */
ASC_DENSE_LAPACK_EXPORT Status
Sytf2(const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
      DenseBlasMatrixView<std::complex<float>> matrix,
      DenseBlasVectorView<index_t> pivots, const LapackWorkspacePlan& plan,
      const LapackWorkspace& workspace, LapackReport& report);

/** @brief Queries double complex SYTF2 workspace without reads.
 * @param provider Explicit checked reference provider; unchanged.
 * @param triangle Selected upper/lower symmetric input triangle.
 * @param matrix Square input/output descriptor; entries are not read here.
 * @param pivots Disjoint contiguous signed ASC output, exact n; entries unread.
...
/** @brief Executes actual double complex SYTF2 Bunch–Kaufman.
 * @param provider Explicit same-build reference selection; no fallback.
 * @param triangle Selected upper/lower symmetric input/output triangle.
 * @param matrix Square input, replaced by completed raw selected block factors.
 * @param pivots Disjoint contiguous n-entry signed one-based block output.
...
/** @brief Queries single complex HETRF workspace without reads.
 * @param provider Explicit checked reference provider; unchanged.
 ...
 * @return Complete formula-bound workspace plan or structural failure.
 */
ASC_DENSE_LAPACK_EXPORT Result<LapackWorkspacePlan> QueryHetrfWorkspace(
    const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
    DenseBlasMatrixView<std::complex<float>> matrix,
    DenseBlasVectorView<index_t> pivots);

/** @brief Executes actual single complex HETRF Bunch–Kaufman.
 * @param provider Explicit same-build reference selection; no fallback.
 ...
 */
ASC_DENSE_LAPACK_EXPORT Status
Hetrf(const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
      DenseBlasMatrixView<std::complex<float>> matrix,
      DenseBlasVectorView<index_t> pivots, const LapackWorkspacePlan& plan,
      const LapackWorkspace& workspace, LapackReport& report);

/** @brief Queries double complex HETRF workspace without reads.
 * @param provider Explicit checked reference provider; unchanged.
 * @param triangle Selected upper/lower Hermitian input triangle.
 * @param matrix Square input/output descriptor; entries are not read here.
 * @param pivots Disjoint contiguous signed ASC output, exact n; entries unread.
...
/** @brief Executes actual double complex HETRF Bunch–Kaufman.
 * @param provider Explicit same-build reference selection; no fallback.
 * @param triangle Selected upper/lower Hermitian input/output triangle.
 * @param matrix Square input, replaced by completed raw selected block factors.
 * @param pivots Disjoint contiguous n-entry signed one-based block output.
...
/** @brief Queries single complex HETF2 workspace without reads.
 * @param provider Explicit checked reference provider; unchanged.
 * @param triangle Selected upper/lower Hermitian input triangle.
 * @param matrix Square input/output descriptor; entries are not read here.
 * @param pivots Disjoint contiguous signed ASC output, exact n; entries unread.
 * @return Complete formula-bound workspace plan or structural failure.
 */
ASC_DENSE_LAPACK_EXPORT Result<LapackWorkspacePlan> QueryHetf2Workspace(
    const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
    DenseBlasMatrixView<std::complex<float>> matrix,
    DenseBlasVectorView<index_t> pivots);

/** @brief Executes actual single complex HETF2 Bunch–Kaufman.
 * @param provider Explicit same-build reference selection; no fallback.
 * @param triangle Selected upper/lower Hermitian input/output triangle.
 * @param matrix Square input, replaced by completed raw selected block factors.
 * @param pivots Disjoint contiguous n-entry signed one-based block output.
 ...
 */
ASC_DENSE_LAPACK_EXPORT Status
Hetf2(const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
      DenseBlasMatrixView<std::complex<float>> matrix,
      DenseBlasVectorView<index_t> pivots, const LapackWorkspacePlan& plan,
      const LapackWorkspace& workspace, LapackReport& report);

/** @brief Queries double complex HETF2 workspace without reads.
 * @param provider Explicit checked reference provider; unchanged.
 * @param triangle Selected upper/lower Hermitian input triangle.
 * @param matrix Square input/output descriptor; entries are not read here.
 * @param pivots Disjoint contiguous signed ASC output, exact n; entries unread.
...
/** @brief Executes actual double complex HETF2 Bunch–Kaufman.
 * @param provider Explicit same-build reference selection; no fallback.
 * @param triangle Selected upper/lower Hermitian input/output triangle.
 * @param matrix Square input, replaced by completed raw selected block factors.
 * @param pivots Disjoint contiguous n-entry signed one-based block output.
...
/** @brief Queries single real SYTRS workspace from metadata.
 ...
 */
ASC_DENSE_LAPACK_EXPORT Result<LapackWorkspacePlan> QuerySytrsWorkspace(
    const ReferenceLapackProvider& provider,
    const ReferenceBunchKaufmanFactorView<float>& factor,
    DenseBlasMatrixView<float> rhs);

/** @brief Solves single real symmetric A*X=B using SYTRS.
 * @param provider Explicit provider matching the factor's copied identity.
 * @param factor Borrowed successful unchanged classic symmetric factor.
 ...
 */
ASC_DENSE_LAPACK_EXPORT Status
Sytrs(const ReferenceLapackProvider& provider,
      const ReferenceBunchKaufmanFactorView<float>& factor,
      DenseBlasMatrixView<float> rhs, const LapackWorkspacePlan& plan,
      const LapackWorkspace& workspace, LapackReport& report);

/** @brief Queries double real SYTRS workspace from metadata.
 * @param provider Explicit same-build reference selection.
 * @param factor Successful symmetric factor and immutable paired pivots.
 * @param rhs Disjoint n-by-nrhs input/output descriptor in either layout.
 * @return Bound pivot/layout formula plan or structural error, without reads
...
/** @brief Solves double real symmetric A*X=B using SYTRS.
 * @param provider Explicit provider matching the factor's copied identity.
 * @param factor Borrowed successful unchanged classic symmetric factor.
 * @param rhs Disjoint n-by-nrhs B, overwritten by X in the original layout.
 * @param plan Complete unchanged matching formula plan.
...
/** @brief Queries single complex SYTRS workspace from metadata.
 * @param provider Explicit same-build reference selection.
 * @param factor Successful symmetric factor and immutable paired pivots.
 * @param rhs Disjoint n-by-nrhs input/output descriptor in either layout.
 * @return Bound pivot/layout formula plan or structural error, without reads
...
/** @brief Solves single complex symmetric A*X=B using SYTRS.
 * @param provider Explicit provider matching the factor's copied identity.
 * @param factor Borrowed successful unchanged classic symmetric factor.
 * @param rhs Disjoint n-by-nrhs B, overwritten by X in the original layout.
 * @param plan Complete unchanged matching formula plan.
...
/** @brief Queries double complex SYTRS workspace from metadata.
 * @param provider Explicit same-build reference selection.
 * @param factor Successful symmetric factor and immutable paired pivots.
 * @param rhs Disjoint n-by-nrhs input/output descriptor in either layout.
 * @return Bound pivot/layout formula plan or structural error, without reads
...
/** @brief Solves double complex symmetric A*X=B using SYTRS.
 * @param provider Explicit provider matching the factor's copied identity.
 * @param factor Borrowed successful unchanged classic symmetric factor.
 * @param rhs Disjoint n-by-nrhs B, overwritten by X in the original layout.
 * @param plan Complete unchanged matching formula plan.
...
/** @brief Queries single complex HETRS workspace from metadata.
 * @param provider Explicit same-build reference selection.
 ...
 * of numerical factors/pivots/RHS or foreign calls.
 */
ASC_DENSE_LAPACK_EXPORT Result<LapackWorkspacePlan> QueryHetrsWorkspace(
    const ReferenceLapackProvider& provider,
    const ReferenceBunchKaufmanFactorView<std::complex<float>>& factor,
    DenseBlasMatrixView<std::complex<float>> rhs);

/** @brief Solves single complex Hermitian A*X=B using HETRS.
 * @param provider Explicit provider matching the factor's copied identity.
 * @param factor Borrowed successful unchanged classic Hermitian factor.
 * @param rhs Disjoint n-by-nrhs B, overwritten by X in the original layout.
 * @param plan Complete unchanged matching formula plan.
 * @param workspace Caller-owned disjoint pivot/layout conversion storage.
 * @param report Mandatory raw INFO and failure-surviving numerical report.
 ...
 */
ASC_DENSE_LAPACK_EXPORT Status
Hetrs(const ReferenceLapackProvider& provider,
      const ReferenceBunchKaufmanFactorView<std::complex<float>>& factor,
      DenseBlasMatrixView<std::complex<float>> rhs,
      const LapackWorkspacePlan& plan, const LapackWorkspace& workspace,
      LapackReport& report);

/** @brief Queries double complex HETRS workspace from metadata.
 * @param provider Explicit same-build reference selection.
 * @param factor Successful Hermitian factor and immutable paired pivots.
 * @param rhs Disjoint n-by-nrhs input/output descriptor in either layout.
 * @return Bound pivot/layout formula plan or structural error, without reads
...
/** @brief Solves double complex Hermitian A*X=B using HETRS.
 * @param provider Explicit provider matching the factor's copied identity.
 * @param factor Borrowed successful unchanged classic Hermitian factor.
 * @param rhs Disjoint n-by-nrhs B, overwritten by X in the original layout.
 * @param plan Complete unchanged matching formula plan.
...
#endif  // ASC_DENSE_PROVIDERS_LAPACK_INDEFINITE_H_
