#ifndef ASC_DENSE_PROVIDERS_LAPACK_TRIANGULAR_PACKED_ERROR_BOUNDS_H_
#define ASC_DENSE_PROVIDERS_LAPACK_TRIANGULAR_PACKED_ERROR_BOUNDS_H_

/** @file
 * @brief Explicit pinned CPU error estimates for a packed triangular solution.
 *
 * TPRFS preserves A, B and the supplied solution X of op(A)*X=B. It computes
 ...
 * narrowing. Active row-major A/B/X need p/n*nrhs/n*nrhs packing entries
 * respectively,where p=n*(n+1)/2. Output estimates are seeded with NaN only
 * after successful preflight. Inputs are never published or overwritten. No
 * singularity scan is introduced. Estimate-quality failures identify the first
 * affected zero-based RHS; negative outputs take precedence over
 * nonfinite-output warnings. Only the selected upper/lower triangle is read.
 * The stored unit diagonal, padding outside the packed span are ignored.
 * Checked packed views retain their complete reachable backing spans; all
 * borrowed objects stay alive. A and RHS layouts are independent. Row-major
 * inputs use explicit live T objects in caller kLayoutConversion storage;
 * column-major inputs are direct. Queries inspect descriptors only, allocate
 * nothing and never call Fortran. Plans bind the routine, scalar, options,
 * original layouts/RHS strides, effective foreign dimensions and exact
 * provider/build/integer ABI. Stale plans fail. All numerical spans, workspace
 * regions and provider/plan/workspace/report metadata must be disjoint.
 * Detected metadata aliases preserve the report; otherwise it is reset before
 * preflight. Structural rejection changes no numerical values or workspace.
 * Accessible host/pinned-host storage and the explicit optional CPU provider
 * are required. No implicit precision change, transfer, fallback,
 * synchronization or allocation occurs in ASC.
 *
 ...
 * before packed division, packed/loop terminals and actual RHS leading
 * dimensions. Workspace values may change after entry. No positive INFO is
 * valid. Negative, missing, partially written or impossible INFO is a provider
 * defect: raw signed INFO survives, outputs are unusable, and the native
 * minimum is never negated. With INFO=0, negative estimates are provider
 * defects; nonnegative nonfinite estimates return kNumerical with
 ...
 * This is ordinary BLAS packed storage, not banded, full or RFP storage.
 * No densification occurs. Link only the explicit optional ASC::dense_lapack
 * facet; these routes provide no additional native capability.
 */
 ...
 */
ASC_DENSE_LAPACK_EXPORT Result<LapackWorkspacePlan> QueryTprfsWorkspace(
...
/** @brief Executes exact pinned TPRFS with the file-level report contract.
 * @param provider Explicit pinned CPU provider borrowed throughout the call.
 * @param triangle Selected upper/lower triangle; invalid enums are rejected.
 * @param diagonal Explicit nonunit or implicit unit diagonal.
 * @param a Immutable ordinary packed triangular matrix; ignored entries stay
...
Tprfs(const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
      DenseBlasDiagonal diagonal, DenseBlasTranspose operation,
      DenseBlasPackedMatrixView<const float> a,
      DenseBlasMatrixView<const float> b, DenseBlasMatrixView<const float> x,
      DenseBlasVectorView<float> ferr, DenseBlasVectorView<float> berr,
...
#endif  // ASC_DENSE_PROVIDERS_LAPACK_TRIANGULAR_PACKED_ERROR_BOUNDS_H_
