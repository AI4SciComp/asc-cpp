...
foreach(_library IN LISTS _libraries)
  ...
  if(NOT _header_result EQUAL 0)
    set(_report "format|asc-cpp-elf-abi-v1\n")
    string(APPEND _report "status|skipped|not-elf|${_library_name}\n")
    file(WRITE "${_output}" "${_report}")
    message(STATUS
  ...
  foreach(_row IN LISTS _soname_rows)
    ...
    string(APPEND _report "soname|${_library_name}|${_soname}\n")
  endforeach()
  string(
    REGEX MATCHALL
    "\\(NEEDED\\)[^\n\r]*\\[[^]]+\\]"
  ...
  foreach(_row IN LISTS _nm_rows)
    ...
    string(REPLACE ";" "\\;" _demangled "${_demangled}")
    list(APPEND _symbols "${_mangled}|${_type}|${_demangled}")
  endforeach()
  list(SORT _symbols)
  foreach(_symbol IN LISTS _symbols)
    string(REGEX MATCH "^([^|]+)\\|([^|]+)\\|(.*)$" _match "${_symbol}")
    if(NOT _match)
      message(FATAL_ERROR
        "Could not parse collected symbol for '${_library_name}'."
      )
    endif()
    set(_mangled "${CMAKE_MATCH_1}")
    set(_type "${CMAKE_MATCH_2}")
    set(_demangled "${CMAKE_MATCH_3}")
    string(REPLACE "\\;" ";" _demangled "${_demangled}")
    string(APPEND _report
