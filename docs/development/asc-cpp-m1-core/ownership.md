...
| --- | --- | --- | --- |
| Lead architect/integrator | root `CMakeLists.txt`, `CMakePresets.json`, `cmake/**`, all module/test CMake registration files when integration requires it, `.github/workflows/ci.yml`, `docs/architecture/dependency-policy.md`, `docs/development/asc-cpp-m1-core/{milestone-contract,ownership,dependency-audit}.md`, manifest/backend-matrix status updates, final integration fixes | entire workspace | all shared build, package, CI, manifest, and integration files |
