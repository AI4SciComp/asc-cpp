...
Status: Milestone 8 Publication Checkpoint B candidate
...
| Provider | Owner/facet | Approved operations through Milestone 8 (unchanged in M8) | Scalars | Indices | Layouts/formats | Platforms | Configure evidence | Compile evidence | Runtime/parity evidence |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| serial reference | provider-free modules plus `random_dense`, `random_sparse`, and `cpp` | prior M4 operations plus deterministic Dense uniform fill and exact-count canonical Sparse generation | prior M4 types; Random facets exactly unqualified float/double | checked signed 64-bit logical metadata and checked unsigned random word offsets | host; logical Dense traversal independent of unique layout; canonical coordinate Sparse output | portable C++20 CPU | configure-tested: CMake 4.1.2; GCC 11.4 and Clang 19; static/shared | compile-tested: strict GCC/Clang facet instantiation and full builds; expected-negative contracts | runtime-tested: corrected GCC Debug/static 193/193, GCC Debug/shared 195/195, and Clang Debug/shared 194/194; ASan+UBSan 139/139; bounded LSan and TSan selections each 12/12; package, relocation, and isolated consumers included |
...
| CUDA Runtime and cuBLAS | `dense_cuda` | bounded terminal/scalar/Negate/Add/Subtract/Multiply evaluation; Copy, Scal, Axpy, Gemv, Gemm | exactly unqualified float/double | ranks 0--8; checked cuBLAS `int` narrowing after full validation | unique nonnegative device views; pointwise positive strides; approved column-major Gemv/Gemm mappings; no hidden packing | NVIDIA GPU; CUDA 12+ | configure-tested: same M6 CUDA configuration | compile-tested: static Debug and shared Release provider/consumer targets; exact element constraints | runtime-tested and parity-tested: independent float/double CPU oracles for evaluator and selected algebra on RTX 3060 Laptop; all transpose pairs, padded/degenerate cases, beta-zero no-read, allocation/concurrency/move-lifetime checks |
