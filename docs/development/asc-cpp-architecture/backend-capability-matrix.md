...
Status: Issue 10 Sparse BLAS Feature Gate B candidate
...
| Provider | Owner/facet | Approved operations through Issue 10 | Scalars | Indices | Layouts/formats | Platforms | Configure evidence | Compile evidence | Runtime/parity evidence |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| serial reference | provider-free modules plus `random_dense`, `random_sparse`, and `cpp` | prior Milestone 8 operations; complete classic Dense BLAS Levels 1, 2, and 3; all 36 applicable Sparse BLAS compute rows | BLAS S/D/C/Z where mathematically defined; Random facets exactly unqualified float/double | checked signed 64-bit logical metadata and checked unsigned random word offsets | host/pinned-host BLAS descriptors; row/column-major full, band, and packed Dense storage; signed vector strides; canonical coordinate/CSR/CSC Sparse storage | portable C++20 CPU | configure-tested: local GCC Debug static/shared Issue 10 builds | compile-tested: strict warnings, installed Sparse consumer, all 36 Sparse rows instantiated | runtime-tested: independent S/D/C/Z Sparse conformance, invalid-input, empty, signed-stride, transpose/conjugation, triangular, and allocation checks |
...
| CUDA Runtime and cuSPARSE | `sparse_cuda` | canonical host indexed/CSR/triangular clones; all 36 applicable asynchronous Sparse BLAS compute rows; retained deterministic CSR SpMV and bounded same-structure coordinate/CSR evaluation | Sparse BLAS exactly S/D/C/Z; compatibility/evaluation exactly unqualified float/double | checked signed 64-bit extents, NNZ, offsets, and indices | trusted zero-based indexed vectors and CSR; signed vector strides; row/column-major Dense operands; no operation workspace; trusted CSC evaluation success skipped | NVIDIA GPU; CUDA 12+ | configure-tested: CMake 4.1.2, CUDA toolkit/compiler 12.9.86, GCC 11.4 host, architecture 86 | compile-tested: local static Release provider and isolated consumer targets; all 36 Sparse rows instantiated | runtime-tested and parity-tested: independent S/D/C/Z indexed, SpMV/SpMM, conjugate-transpose, triangular solve, signed-stride, invalid-memory, completion, and allocation-contract evidence on RTX 3060 Laptop |
