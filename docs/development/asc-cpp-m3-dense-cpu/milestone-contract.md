...
Status: Frozen from the owner-approved Milestone 3 contract
...
Predecessor state: the intentionally uncommitted and locally validated
Milestones 0--2 Publication Checkpoint B candidate and the user's prior
deletion of the retired implementation are retained in place. This milestone
does not restore, reset, discard, or reclassify that work.

## Authority and purpose

This contract is subordinate to the owner's current instruction, the all-in-one
runbook, the approved architecture package and ADRs, and the exact previously
owner-approved Milestone 3 contract retained on the completed hardening branch.
No later implementation was inspected or adopted to prepare this continuation.

Implement only the approved CPU foundation of the `dense` module. The
component owns multidimensional dense storage, views, layout mappings, dense
expression evaluation, selected reductions, and the narrow deterministic
serial dense linear-algebra baseline.
...
## Exact public and compiled files
...
The exact compiled-source inventory, frozen after the production API was
signature-complete and before lead integration, is:

```text
src/dense/linalg.cc
```

Public headers are self-contained `.h` files. Compiled C++ sources are `.cc`
files. No production source outside `src/dense/` is approved.
...
- Mutable views and owners require a mapping whose uniqueness is proven.
  Milestone 3 may conservatively reject a mathematically unique
  arbitrary-stride mapping when the proof is not available.
- Owners require unique and exhaustive storage. Padded/non-exhaustive mappings
  are view-only in Milestone 3.
...
- an element pointer;
- a validated layout mapping; and
...
`ExpressionAdapter`; Expression does not include a Dense header. The adapter
reports shape, value type, rank, terminal category, structure-preserving
sparsity, value reads, and conservative alias identity.
...
- is move-only;
- owns host-accessible, unique, exhaustive storage in Milestone 3;
...
Milestone 3 owner elements are arithmetic, trivially copyable, and trivially
...
- direct exact-view self-assignment is a permitted no-op; and
- every other possible destination overlap is conservatively rejected before
  mutation in Milestone 3.

Selected reductions are deterministic logical-order `ReduceSum`, `ReduceMin`,
and `ReduceMax`. Empty sum returns zero; empty minimum/maximum return
`kInvalidArgument`. No parallel reduction or order-independent result is
claimed.
...
They support `float` and `double`, an explicit serial `ExecutionContext`,
rank-one or rank-two dense views, arbitrary validated non-negative strides,
and caller-provided destinations. `Gemv` and `Gemm` support `kNone` and
...
- independently calculated numerical cases for every linear-algebra operation,
...
- build-tree, installed, relocated, path-with-spaces, static/shared,
  subproject, and Dense-only consumer evidence; and
...
- No production change to Core, Utilities, Expression, or Random except a
  release-blocking compatibility correction discovered by review and recorded
  explicitly by the lead.
...
pass/fail/skip results. Remote writes require separate owner approval.
