...
All write scopes are disjoint. Every role preserves the cumulative M0--M7
working tree and unrelated owner work. No role may commit, push, merge, tag,
release, delete a branch, add an unapproved dependency, modify another
repository, or implement a later milestone.
...
component package integration, root/shared CMake, architecture manifests,
test registration, complete diff review, clean matrix execution, final
evidence classification, and Checkpoint B. Only the lead edits root, shared
CMake, package, or architecture files.
...
inventory and reporting tools plus reviewed local baselines. Tools must be
read-only with respect to source/product artifacts, deterministic, CMake/shell
portable within the declared support matrix, and introduce no product
...
public API, installed-header, symbol, compile-time/object-size, and asc-xde
trial checks. It must use independent oracles and only public downstream APIs.
It does not edit production tools, product code, CMake/package integration,
architecture files, general docs, or another repository.
...
Responsibilities: document and independently audit the M8 package metadata,
source/ABI/provider compatibility boundaries, support matrix, extension rules,
asc-xde trial, performance envelope, ownership/lifetime implications, and
unreleased status. API/package defects are reported, not documented around.
...
If a role needs work outside its scope, it reports the exact file/symbol and
reason. Only the lead may make a shared integration change or revise scope,
and any revision is recorded before the write.
