...
Status: accepted 0.9.0 Random provenance record
...
Random engines and distributions destination baseline:
`47367b9e79fde8cd633e8e91e2eeb41fb751e1ec`

Quasi-random sequences destination baseline:
`ea2ac5539d20944f6d012cebf1e8ed6d6157a554`

Random storage adapters destination baseline:
`499afc7073435d6c5eb51b188cf7bdb722dfa8a6`

Random completion evidence baseline:
...
The Random release contract confirms that all 33 inventory decisions
...
Random engines and distributions completes the recommended path: the selected older variants are
...
| `PROV-03` | **Implemented by Random engines and distributions:** SplitMix64 2015, PCG32 minimal C 0.9 XSH-RR, xoroshiro64* 1.0, and xoroshiro128+ 1.0 retain the Random contract URL/SHA identities and independently derived vectors | No blocker for these four rows; later algorithms retain their own routes |
| `PROV-04` | **Implemented by Quasi-random sequences:** the Joe/Kuo license is retained in source/install artifacts and reproduced in `THIRD_PARTY_NOTICES`; no endorsement is claimed | Other accepted external source/data still needs a component-specific owner decision |
| `PROV-05` | **Implemented by Quasi-random sequences:** original Sobol recurrence uses only the pinned BSD-style `new-joe-kuo-6.21201` input; input, license, generator, generated source, install, and every compiled direction row have identity checks | No blocker for the Quasi-random sequences data row; all MdeCpp/Burkardt Sobol artifacts remain rejected |
