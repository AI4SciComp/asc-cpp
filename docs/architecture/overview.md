...
BSD-style `new-joe-kuo-6.21201` input for Quasi-random sequences. The historical MdeCpp
...
## 16. Change and release boundary

The durable architecture is controlled by the accepted ADRs and the contracts
under `docs/contracts/`. Changes to module edges, public components,
compatibility policy, provider status, or provenance require an ADR and matching
contract/test updates. The release process in `docs/release-process.md` governs
validation and publication.

The provider-free C++20 surface is supported for 0.9.0. CUDA providers remain
explicit experimental components. MdeCpp is a behavioral catalogue only;
copied MdeCpp implementation or test material is not approved.
