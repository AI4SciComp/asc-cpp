...
Status: Approved by BLAS contract; implementation completed through Sparse BLAS and
independently audited by BLAS completion
...
Before BLAS rename, ASCCpp exposed a small float/double linear-algebra subset through
...
This decision is Class C. It freezes the public surface, numerical and
execution semantics, provider boundaries, standards inventory, compatibility
route, coverage evidence, and implementation order used for the completed
rename and operation inventory.
...
BLAS rename implements the approved breaking rename in the next pre-1.0 minor:
...
target, component, or deprecation window. The rename landed before the first
public `0.9.0` release, and the approved compatibility policy permits that
documented pre-1.0 minor break. Migration notes give mechanical
include/type/source renames while preserving legitimate prose such as “linear
algebra.” Historical records remain historical and are not blindly rewritten.
...
`docs/contracts/blas-coverage.yaml` is the source of
...
redefine the validator's BLAS contract contract. It deterministically generates
...
release verification evidence must show that each declared verified backend actually
...
1. BLAS rename performs only the approved breaking rename, migration notes,
   packaging/header tests, and manifest link updates.
2. Dense BLAS Levels 1, 2, and 3 are implemented and verified.
3. Sparse BLAS implements and verifies the applicable Sparse BLAS compute
   families without introducing a Dense dependency.
4. BLAS completion independently audits the frozen inventories, manifest/API/source/
   test links, numerical evidence, packaging, documentation, and performance.

The first implementation branch, BLAS rename, is expected to change only the
