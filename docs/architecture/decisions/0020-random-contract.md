...
Status: Accepted; Random engines, quasi-random sequences, storage adapters,
and completion evidence are implemented and audited

Date: 2026-08-01

## Context

This decision freezes the architecture, provenance, and verification contract
implemented by Random engines, quasi-random sequences, and storage adapters.
It does not itself add an engine, distribution, sampler, helper, adapter,
target, dependency, or public API.
...
- Quasi-random sequences must retain that license in source and binary distributions, add
...
- `jump` and `long_jump` use only the pinned version-1 polynomials. Arbitrary
  runtime jump-polynomial machinery is not approved in Random engines and distributions.
...
- Scalar normal version 1 obtains `U1` and `U2` from those unit mappings and
  ...
  order must appear in Random engines and distributions known-answer tests. Transcendental results are
...
The Random engine, quasi-random, and storage-adapter contracts approve only the
portable serial CPU path for the new
...
The release contract requires at least these frozen tests:
...
normal. Product files are `engine.h/.cc`, `distribution.h/.cc`,
`seed.h/.cc`, and `generator.h`. Verification includes
...
and Sobol algorithms. Product/data files are `quasi.h/.cc`, the pinned
...
adapters belong to Random storage adapters.

### Random storage adapters
...
Latin/Halton/Hammersley/Sobol algorithms, existing-pattern Sparse value fill,
structure-only Sparse generation, and combined Sparse object generation.
Product changes are confined to `random/dense.h`, `random/sparse.h`, and
`src/random/CMakeLists.txt`. Verification includes
...
Completion evidence independently proves that every approved inventory item
has a compatible provenance route, documented semantics, an existing
implementation and owning build registration, reproducible tests, public
documentation, package consumers, and correctness-guarded performance
evidence. It is confined to manifests, generated reports, architecture checks,
...
Any later API refinement must remain within the named files and semantics
without changing ownership, dependencies, provenance, state mapping, or
compatibility. A new architecture decision is required if it needs:
