#ifndef ASC_TESTS_EXPRESSION_TEST_SUPPORT_H_
#define ASC_TESTS_EXPRESSION_TEST_SUPPORT_H_

#include <iostream>
#include <string_view>

namespace asc_expression_test {
...
#define ASC_EXPRESSION_TEST_CHECK(context, expression)                  \
  (context).Check(static_cast<bool>(expression), #expression, __FILE__, \
                  __LINE__)

#define ASC_EXPRESSION_TEST_EQ(context, left, right) \
  (context).CheckEqual((left), (right), #left, #right, __FILE__, __LINE__)

#endif  // ASC_TESTS_EXPRESSION_TEST_SUPPORT_H_
