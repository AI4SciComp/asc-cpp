...
void Case(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  asc_block_inverse_test::Native(sample.hermitian, &triangle, &n, native.data(),
                                 &n, pivots.data(), work.data(), &info);
...
int Run(bool hermitian, bool fidelity) {
  ...
  std::printf(
      "block inverse range cases=%d representable=%d nonrepresentable=%d "
      "fidelity=%d\n",
...
int main(int argc, char** argv) {
  ...
  if (argc != 4 || !asc_block_inverse_test::Select(argv[3])) {
    return 2;
  }
