...
struct Sample {
  ...
  void CheckFactor(TestContext& test) const {
    ...
    for (int j = 0; j < mode.n; ++j) {
      ...
      const int paired = mode.triangle == support::kUpper ? -1 : -2;
...
std::string_view FactorRoutine(bool hermitian, bool blocked) {
  if (hermitian) {
    return blocked ? "hetrf" : "hetf2";
  }
  return blocked ? "sytrf" : "sytf2";
}
...
auto Borrow(const asc::ReferenceLapackProvider& provider,
  ...
  return Take(asc::ReferenceBunchKaufmanFactorView<T>::Create(
  ...
  for (int repeat = 0; repeat < kRepeats; ++repeat) {
    ...
    ASC_DENSE_TEST_CHECK(
        test, support::Solve(provider, Hermitian, factor, rhs.View(), plan,
                             workspace, report)
                  .ok());
    CheckSuccess(test, report, provider, Hermitian ? "hetrs" : "sytrs");
    rhs.Check(test);
...
void Worker(TestContext& test, int worker, const Mode& mode,
            ...
            const asc::ReferenceBunchKaufmanFactorView<T>& shared_factor,
            std::barrier<>& rendezvous) {
