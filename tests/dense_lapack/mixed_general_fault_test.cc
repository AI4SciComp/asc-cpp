...
namespace fault = asc_mixed_fault;
using support::Take;
...
void Run(TestContext& test, const asc::ReferenceLapackProvider& provider) {
  for (const auto mode :
       {fault::Mode::kNegativeInfo, fault::Mode::kExcessInfo,
        ...
        fault::Mode::kPartialIter, fault::Mode::kInvalidPivot,
        fault::Mode::kUnwrittenPivot, fault::Mode::kPartialPivot,
        fault::Mode::kWriteThenNegative, fault::Mode::kUnwrittenX,
        fault::Mode::kImplementationFallback}) {
    for (const auto al : {support::kRow, support::kColumn}) {
      for (const auto xl : {support::kRow, support::kColumn}) {
        support::Problem<T> p(3, 2, al, support::kColumn, xl);
        const auto before = p;
        ...
        if (al == support::kColumn && mode == fault::Mode::kWriteThenNegative) {
