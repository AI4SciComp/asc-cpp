...
struct Sample {
  ...
  Sample(int mode, bool he, asc::DenseBlasTriangle tri,
    ...
    pivots.fill(-433);
    extra.fill(support::Value<T>(-439, 23));
...
template <typename T>
std::array<T, 49> NativeInverse(TestContext& test, const Sample<T>& sample) {
  ...
  asc_rk_inverse_test::Native(sample.hermitian, &triangle, &n, native.data(),
                              &n, sample.extra.data() + 1, pivots.data(),
                              work.data(), &info);
...
void Case(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  auto a = sample.View();
  const auto pivot_view =
      asc_indefinite_rook_test::Pivots(sample.pivots, sample.n);
  auto extra = asc_rk_test::OffDiagonal(sample.extra, sample.n);
  const auto factor_plan = support::Take(
      asc_rk_test::QueryFactor(provider, sample.triangle, sample.hermitian,
                               blocked, a, extra, pivot_view));
  ...
  const auto factor_status = asc_rk_test::Factor(
      provider, sample.triangle, sample.hermitian, blocked, a, extra,
  ...
  const auto raw = asc_indefinite_rook_test::Raw(sample.pivots, sample.n);
  ...
  const auto native = NativeInverse(test, sample);
  ...
  ASC_DENSE_TEST_CHECK(
      test, support::EqualBytes(sample.extra.data(), extra_before.data(),
                                sizeof(extra_before)));
...
int Run(bool hermitian, bool fidelity) {
  ...
  std::printf(
      "RK inverse range cases=%d representable=%d nonrepresentable=%d "
      "fidelity=%d\n",
...
int main(int argc, char** argv) {
  ...
  if (argc != 4 || !asc_rk_inverse_test::Select(argv[3])) {
    return 2;
  }
