...
void Fidelity(TestContext& test, const Sample<T>& sample) {
  ...
  std::array<lapack_int, 3> info{-103, std::numeric_limits<lapack_int>::min(),
                                 -103};
  asc_block_solve_test::Native(sample.hermitian, &uplo, &n, &nrhs, a.data() + 1,
                               &n, pivots.data() + 1, b.data() + 1, &n,
                               work.data() + 1, info.data() + 1);
...
void Case(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  const auto factor_plan = Take(support::QueryFactor(
  ...
  const auto factored = support::Factor(
      provider, sample.triangle, sample.hermitian, false, sample.AView(),
  ...
  const auto plan = Take(support::WithoutAllocation(test, [&] {
    return asc_block_solve_test::Query(provider, sample.triangle,
                                       sample.hermitian, sample.ConstAView(),
                                       raw, sample.BView());
  }));
  ...
  const auto status = support::WithoutAllocation(test, [&] {
    return asc_block_solve_test::Solve(
        provider, sample.triangle, sample.hermitian, sample.ConstAView(), raw,
  ...
  std::printf(
      "block solve range n=%d he=%d tri=%d A_layout=%d B_layout=%d scale=%La "
...
int Run(bool hermitian, bool mathematical) {
  ...
  std::printf(
      "block solve range cases=%d representable=%d nonrepresentable=%d\n",
