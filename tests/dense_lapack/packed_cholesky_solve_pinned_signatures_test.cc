...
// Hand-reviewed complete PPTRS signatures, checked separately against GNU
...
template <typename T>
using Solve = void(char*, Integer*, Integer*, T*, T*, Integer*, Integer*,
                   std::size_t);
static_assert(kMatches<decltype(LAPACK_spptrs_base), Solve<float>>);
static_assert(kMatches<decltype(LAPACK_dpptrs_base), Solve<double>>);
static_assert(
    kMatches<decltype(LAPACK_cpptrs_base), Solve<std::complex<float>>>);
static_assert(
    kMatches<decltype(LAPACK_zpptrs_base), Solve<std::complex<double>>>);
...
int main() {
  ...
  std::printf(
      "4 complete packed Cholesky solve signatures passed; configured "
