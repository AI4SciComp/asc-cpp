...
namespace {
using asc_packed_cholesky_inverse_test::Fault;
...
class Checks {
 public:
  void Expect(bool condition, const char* message) {
    ...
    if (!condition) {
      if (failures_ < 30) {
        std::fprintf(stderr, "Packed Cholesky inverse failure: %s\n", message);
      }
      ++failures_;
    }
  }
  ...
  [[nodiscard]] int Finish() const {
    std::printf(
        "Packed Cholesky inverse failure acceptance: %zu profiles, %zu checks, "
        "%zu "
        "failures\n",
...
template <typename T>
auto ExpectedName() {
  constexpr std::array<std::string_view, 4> kNames{"spptri", "dpptri", "cpptri",
                                                   "zpptri"};
...
struct Fixture {
  ...
  auto Query(const asc::ReferenceLapackProvider& provider,
    ...
    return asc::QueryPptriWorkspace(provider, profile.triangle, View(profile));
  }
  ...
  asc::Status Call(const asc::ReferenceLapackProvider& provider,
    ...
    return asc::Pptri(provider, profile.triangle, View(profile), plan,
                      workspace, report);
  }
};
...
void Inject(Checks& checks, const asc::ReferenceLapackProvider& provider,
  ...
  asc::LapackReport report;
  asc_packed_cholesky_inverse_test::Arm(routine, fault);
  const auto calls = asc_packed_cholesky_inverse_test::Calls(routine);
  ...
  asc_packed_cholesky_inverse_test::Disarm();
  ...
  checks.Expect(asc_packed_cholesky_inverse_test::Calls(routine) == calls + 1,
                "exact selected native route entered once");
  CheckReport(checks, report, status, fault);
  const auto arguments =
      asc_packed_cholesky_inverse_test::LastArguments(routine);
  checks.Expect(
...
std::size_t TotalCalls() {
  ...
  for (std::size_t i = 0; i < 4; ++i) {
    total += asc_packed_cholesky_inverse_test::Calls(i);
  }
  return total;
}
...
void RealCalls(Checks& checks, const asc::ReferenceLapackProvider& provider,
  ...
  for (int repeat = 0; repeat < 2; ++repeat) {
    ...
    checks.Expect(asc_packed_cholesky_inverse_test::Calls(routine) != 0 &&
                      TotalCalls() == calls + 1,
