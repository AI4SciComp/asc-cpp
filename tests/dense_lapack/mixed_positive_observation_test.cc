...
#include "../../src/dense/lapack/internal_mixed_positive_counts.h"
...
void ObserveCase(TestContext& test,
                 const asc::ReferenceLapackProvider& provider, Page<T>& a,
                 Page<T>& b, Page<T>& x, asc::DenseBlasLayout al,
  ...
  const auto call = [&](const asc::LapackWorkspacePlan& selected,
    ...
    if constexpr (asc::DenseBlasComplex<T>) {
      return asc::Zcposv(provider, av, uplo, bv, xv, selected, work, statistics,
                         report);
    } else {
      return asc::Dsposv(provider, av, uplo, bv, xv, selected, work, statistics,
                         report);
    }
  };
  a.Protect(PROT_NONE);
  ...
  asc_mixed_positive_fault::Reset(asc_mixed_positive_fault::Mode::kPass);
  ASC_DENSE_TEST_CHECK(test, !call(stale, workspace).ok());
  ASC_DENSE_TEST_EQ(test, asc_mixed_positive_fault::Calls(), std::size_t{0});
  if (n > 0) {
  ...
  if (n == 0) {
    ...
    ASC_DENSE_TEST_EQ(test, asc_mixed_positive_fault::Calls(), std::size_t{0});
    ASC_DENSE_TEST_CHECK(test,
                         !report.native_info && !statistics.native_iteration);
  }
  ...
  if (n > 0) {
    g_outputs = {x.Storage()};
    asc_mixed_positive_fault::OnEntry(RestoreAtEntry);
    ASC_DENSE_TEST_CHECK(test, call(plan, workspace).ok());
    ASC_DENSE_TEST_EQ(test, asc_mixed_positive_fault::Calls(), std::size_t{1});
      ...
      for (const auto xl : {kColumn, kRow}) {
        for (const asc::extent_t n : {0, 3}) {
          for (const asc::extent_t nrhs : {0, 2}) {
            ObserveCase(test, provider, a, b, x, al, bl, xl, n, nrhs, uplo);
          }
        }
      }
    }
...
void Counts(TestContext& test) {
  namespace counts = asc::internal_mixed_positive_counts;
...
int main() {
  ...
  Counts(test);
  Observe<double>(test, provider, asc::DenseBlasTriangle::kUpper);
  Observe<double>(test, provider, asc::DenseBlasTriangle::kLower);
  Observe<std::complex<double>>(test, provider, asc::DenseBlasTriangle::kUpper);
  Observe<std::complex<double>>(test, provider, asc::DenseBlasTriangle::kLower);
  std::puts(
      "Protected initialized X until wrapped native entry; both mixed "
      "drivers, "
