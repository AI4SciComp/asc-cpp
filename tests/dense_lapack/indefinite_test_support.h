#ifndef ASC_TESTS_DENSE_LAPACK_INDEFINITE_TEST_SUPPORT_H_
#define ASC_TESTS_DENSE_LAPACK_INDEFINITE_TEST_SUPPORT_H_
...
constexpr auto kBlock = asc::LapackFactorFamily::kBunchKaufman;
...
auto QuerySolve(const asc::ReferenceLapackProvider& provider, bool hermitian,
  ...
  if constexpr (asc::DenseBlasComplex<T>) {
    if (hermitian) {
      return asc::QueryHetrsWorkspace(provider, factor, rhs);
    }
  }
  return asc::QuerySytrsWorkspace(provider, factor, rhs);
}
...
asc::Status Solve(const asc::ReferenceLapackProvider& provider, bool hermitian,
  ...
  if constexpr (asc::DenseBlasComplex<T>) {
    if (hermitian) {
      return asc::Hetrs(provider, factor, rhs, plan, workspace, report);
    }
  }
  return asc::Sytrs(provider, factor, rhs, plan, workspace, report);
}
...
#endif  // ASC_TESTS_DENSE_LAPACK_INDEFINITE_TEST_SUPPORT_H_
