...
// Hand-reviewed complete PPSV signatures, checked separately against GNU
...
using Solve = void(char*, Integer*, Integer*, T*, T*, Integer*, Integer*,
                   std::size_t);
static_assert(kMatches<decltype(LAPACK_sppsv_base), Solve<float>>);
static_assert(kMatches<decltype(LAPACK_dppsv_base), Solve<double>>);
static_assert(
    kMatches<decltype(LAPACK_cppsv_base), Solve<std::complex<float>>>);
static_assert(
    kMatches<decltype(LAPACK_zppsv_base), Solve<std::complex<double>>>);
...
int main() {
  ...
  std::printf(
      "4 complete packed Cholesky driver signatures passed; configured "
