#ifndef ASC_TESTS_DENSE_LAPACK_LU_EXPERT_INFO_TEST_SUPPORT_H_
#define ASC_TESTS_DENSE_LAPACK_LU_EXPERT_INFO_TEST_SUPPORT_H_
#include <algorithm>
#include <array>
#include <bit>
#include <cmath>
...
using Fault = asc_lapack_test::LuExpertInfoFault;
...
inline auto Pivots(std::array<asc::index_t, 6>& values, int count) {
  return Take(asc::DenseBlasVectorView<asc::index_t>::Create(
      values.data() + 1, count, 1, {values.data(), sizeof(values), kHost}));
}
inline auto Raw(const std::array<asc::index_t, 6>& values, int count) {
...
#endif  // ASC_TESTS_DENSE_LAPACK_LU_EXPERT_INFO_TEST_SUPPORT_H_
