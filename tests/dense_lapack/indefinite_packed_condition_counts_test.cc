...
#include "../../src/dense/lapack/internal_indefinite_packed_condition_counts.h"
...
int main() {
  ...
  using asc::internal_indefinite_packed_condition_counts::Condition;
  for (const asc::extent_t limit :
    ...
    for (const asc::extent_t n :
      ...
      ASC_DENSE_TEST_CHECK(test, Condition(n, false, limit).ok());
    }
    for (const asc::extent_t n :
         std::array<asc::extent_t, 4>{bound + 1, limit - 1, limit, INT64_MAX}) {
      ASC_DENSE_TEST_EQ(test, Condition(n, false, limit).code(),
                        asc::ErrorCode::kOverflow);
    }
    ASC_DENSE_TEST_CHECK(test, Condition(limit, true, limit).ok());
    ASC_DENSE_TEST_CHECK(test, Condition(INT64_MAX, true, limit).ok());
    ASC_DENSE_TEST_EQ(test, Condition(-1, false, limit).code(),
                      asc::ErrorCode::kInvalidArgument);
  }
  for (const asc::extent_t limit : std::array<asc::extent_t, 3>{-1, 0, 17}) {
    ASC_DENSE_TEST_EQ(test, Condition(0, false, limit).code(),
                      asc::ErrorCode::kInvalidArgument);
  }
  return test.Finish();
}
