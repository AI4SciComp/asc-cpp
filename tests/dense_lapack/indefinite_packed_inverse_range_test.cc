...
struct Sample {
  ...
  [[nodiscard]] std::size_t Offset(int i, int j) const {
    ...
    if (layout == support::kColumn) {
      k = triangle == support::kUpper ? j * (j + 1) / 2 + i
                                      : j * (2 * n - j + 1) / 2 + i - j;
    } else {
      k = triangle == support::kUpper ? i * (2 * n - i + 1) / 2 + j - i
                                      : i * (i + 1) / 2 + j;
    }
    return 1U + static_cast<std::size_t>(k);
  }
  auto View() {
    return support::Take(asc::DenseBlasPackedMatrixView<T>::Create(
        a.data() + 1, n, layout, {a.data(), sizeof(a), support::kHost}));
  }
...
void Case(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  const auto factor_plan = support::Take(asc_packed_indefinite_test::Query(
      provider, sample.triangle, sample.hermitian, a, pivot_view));
  ...
  const auto factor_status = asc_packed_indefinite_test::Factor(
      provider, sample.triangle, sample.hermitian, a, pivot_view, factor_plan,
      factor_work, factor_report);
  ...
  const auto plan = support::Take(support::WithoutAllocation(test, [&] {
    return asc_packed_inverse_test::Query(provider, sample.triangle,
  ...
  for (int j = 0; j < sample.n; ++j) {
    ...
    for (int i = 0; i < sample.n; ++i) {
      if (sample.Selected(i, j)) {
        native[slot++] = sample.a[sample.Offset(i, j)];
      }
    }
  }
  ...
  asc_packed_inverse_test::Native(sample.hermitian, &triangle, &n,
                                  native.data(), pivots.data(), work.data(),
                                  &info);
  asc::LapackReport report;
  const auto status = support::WithoutAllocation(test, [&] {
    return asc_packed_inverse_test::Inverse(provider, sample.triangle,
  ...
  if (fidelity) {
    ...
    for (int j = 0; j < sample.n; ++j) {
      for (int i = 0; i < sample.n; ++i) {
        if (sample.Selected(i, j)) {
          ASC_DENSE_TEST_CHECK(
              test, Fidelity(sample.a[sample.Offset(i, j)], native[slot++]));
        }
      }
    }
  ...
  for (std::size_t k = 0; k < sample.a.size(); ++k) {
    if (k == 0 || k > static_cast<std::size_t>(sample.n * (sample.n + 1) / 2)) {
      ASC_DENSE_TEST_CHECK(
...
int Run(bool hermitian, bool fidelity) {
  ...
  ASC_DENSE_TEST_EQ(test, cases, 72);
  ASC_DENSE_TEST_EQ(test, representable, 60);
  std::printf(
      "packed inverse range cases=%d representable=%d nonrepresentable=%d "
      "fidelity=%d\n",
