...
template <typename T>
Sample<T> PrepareInfo(TestContext& test,
                      const asc::ReferenceLapackProvider& provider,
                      const InfoMode& mode, Sample<T>& original) {
  using Real = asc::DenseBlasRealType<T>;
...
template <typename T>
void CheckInfoPublication(TestContext& test, const InfoMode& mode, Fault fault,
                          const Sample<T>& before, const Sample<T>& positive,
                          const Sample<T>& after) {
  const bool defect = mode.n != 0 && fault != Fault::kNone;
...
template <typename T>
void CheckInfoNumerics(TestContext& test, const InfoMode& mode,
...
void InfoCase(TestContext& test, const asc::ReferenceLapackProvider& provider,
              const InfoMode& mode, char scalar) {
  Sample<T> original(mode.n, mode.nrhs, mode.layouts);
  const auto before = PrepareInfo(test, provider, mode, original);
  auto positive = before;
  int actual = 0;
  if (mode.kind == 0) {
    actual = 2;
  } else if (mode.kind == 1 && mode.fact != 'E') {
    actual = 4;
  }
  auto outcome = asc::LapackOutcome::kSuccess;
  if (actual != 0) {
    outcome = mode.kind == 0 ? asc::LapackOutcome::kSingular
  ...
  for (auto fault : {Fault::kNone, Fault::kWithhold, Fault::kShortZero}) {
    ...
    std::printf(
        "LU_AUX_MODE family=driver scalar=%c n=%d nrhs=%d layouts=%u "
        "trans=%d fact=%c equed=%d kind=%d fault=%d\n",
    ...
    aux::CheckInfo(test, provider, status, report, mode.n != 0, fault,
                   aux::Name(scalar, "gesvx").data(), actual, outcome,
    ...
    if (fault == Fault::kNone) {
      CheckInfoNumerics(test, mode, original, before, sample);
      positive = sample;
    }
    CheckInfoPublication(test, mode, fault, before, positive, sample);
...
int main(int argc, char** argv) {
  ...
  if (scalar == "s") {
    driver::RunInfo<float>(test, provider, 's');
  } else if (scalar == "d") {
    driver::RunInfo<double>(test, provider, 'd');
  } else if (scalar == "c") {
    driver::RunInfo<std::complex<float>>(test, provider, 'c');
  } else if (scalar == "z") {
    driver::RunInfo<std::complex<double>>(test, provider, 'z');
  } else {
    return 2;
  }
  std::printf("LU_AUX_COMPLETED family=driver scalar=%s abi=%d cases=%zu\n",
