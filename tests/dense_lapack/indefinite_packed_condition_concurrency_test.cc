...
void Worker(TestContext& test, const asc::ReferenceLapackProvider& provider,
            const condition::Fixture<T>& sample,
            asc::DenseBlasRealType<T> expected,
  ...
  for (int repeat = 0; repeat < 4; ++repeat) {
    ...
    const auto status = condition::Condition(
        provider, sample.a.triangle, sample.a.hermitian, factors, pivots,
        sample.norm, result[1], plan, workspace, report);
    ...
    ASC_DENSE_TEST_EQ(test, result[1], expected);
    ASC_DENSE_TEST_EQ(test, result.front(), Real{-157});
    ASC_DENSE_TEST_EQ(test, result.back(), Real{-157});
    const auto scratch_before = scratch;
    start.arrive_and_wait();
    const auto rejected = condition::Condition(
        provider, sample.a.triangle, sample.a.hermitian, factors, pivots,
        sample.norm, result[1], stale_plan, workspace, report);
...
void Group(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  const auto factors =
      condition::Take(asc::DenseBlasPackedMatrixView<const T>::Create(
          protected_a.data() + 1, sample.a.n, sample.a.layout,
          {protected_a.data(), sizeof(sample.a.a), base::kHost}));
  const auto pivots = condition::Take(asc::RawLapackPivotView::Create(
      protected_p.data() + 1, sample.a.n,
      asc::LapackFactorFamily::kBunchKaufman,
      {protected_p.data(), sizeof(sample.a.pivots), base::kHost}));
  asc::DenseBlasRealType<T> serial = -157;
  const auto plan = condition::Take(
      condition::Query(provider, sample.a.triangle, sample.a.hermitian, factors,
                       pivots, sample.norm, serial));
  ...
  asc::LapackReport report;
  ASC_DENSE_TEST_CHECK(
      test, condition::Condition(provider, sample.a.triangle,
                                 sample.a.hermitian, factors, pivots,
                                 sample.norm, serial, plan, work, report)
                .ok());
  sample.Mathematics(test, serial);
  ...
  for (std::size_t i = 0; i < threads.size(); ++i) {
    threads[i] = std::thread([&, i] {
      Worker(results[i], provider, sample, serial, factors, pivots, plan,
             stale_plan, start);
    });
  }
...
int Run(bool hermitian) {
  TestContext test;
  const auto provider = condition::Take(
      asc::ReferenceLapackProvider::Create(asc::ExecutionContext::Serial()));
  int groups = 0;
  for (int n : {0, 1, 2, 5, 17, 65}) {
    for (int kind : {0, 1, 2, 3}) {
      for (auto triangle : {condition::kUpper, condition::kLower}) {
        for (auto layout : {condition::kColumn, condition::kRow}) {
          Group(test, provider,
                condition::Fixture<T>(n, hermitian, triangle, layout, 0, kind));
          ++groups;
        }
      }
    }
  }
  ASC_DENSE_TEST_EQ(test, groups, 96);
  std::printf("Packed condition concurrent groups=%d workers=4 repeats=4\n",
              groups);
  return test.Finish();
}
}  // namespace
