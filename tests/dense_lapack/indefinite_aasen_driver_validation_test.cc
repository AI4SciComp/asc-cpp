...
#include "../../src/dense/lapack/internal_indefinite_aasen_driver_counts.h"
...
using asc_aasen_driver_test::Driver;
using asc_aasen_driver_test::Query;
...
void AlterOperands(int bad, asc::DenseBlasTriangle triangle,
  ...
  switch (bad) {
    ...
    case 21:
      ...
      break;
    case 22:
      matrix = base::Matrix(a, 2, 3, layout, 3);
      break;
    case 23:
      matrix = base::Matrix(a, 2, 2, layout, 5);
      break;
    case 24:
      matrix = base::Matrix(
          a, 2, 2, layout == base::kColumn ? base::kRow : base::kColumn, 3);
      break;
    case 27:
      pivots = base::Take(asc::DenseBlasVectorView<asc::index_t>::Create(
      ...
      break;
    case 28:
      matrix = base::Matrix(a, 1, 1, layout, 3);
      break;
    case 25:
      pivots = base::Take(asc::DenseBlasVectorView<asc::index_t>::Create(
...
void AlterExtra(int bad, const asc::ReferenceLapackProvider& provider,
  ...
  switch (bad) {
    ...
    case 26:
      ...
      break;
    case 41:
      matrix = base::Matrix(b, 2, 2, base::kColumn, 3);
      break;
    case 42:
      workspace.regions[base::kScalar] = {
    ...
    case 45:
      matrix = base::Take(asc::DenseBlasMatrixView<T>::Create(
          ...
          2, 2, base::kColumn, 2, {&provider, sizeof(provider), base::kHost}));
      break;
    case 46:
      matrix = base::Take(asc::DenseBlasMatrixView<T>::Create(
          reinterpret_cast<T*>(&plan), 2, 2, base::kColumn, 2,
...
void Rejection(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  ASC_DENSE_TEST_EQ(test, plan.regions[base::kLayout].minimum_entries,
                    (hermitian || layout == base::kRow ? 4 : 0) +
                        (rhs_layout == base::kRow ? 4 : 0));
...
int OriginalStrides(TestContext& test,
                    ...
                    bool hermitian, T* a, asc::index_t* p, T* b) {
  constexpr auto kLimit = asc::internal_indefinite::kIntegerLimit;
  const auto pivots = base::Take(asc::DenseBlasVectorView<asc::index_t>::Create(
      p, 1, 1, {p, sizeof(asc::index_t), base::kHost}));
  int checks = 0;
...
void Counts(TestContext& test) {
  namespace counts = asc::internal_indefinite_aasen_driver_counts;
  for (const asc::extent_t limit : {static_cast<asc::extent_t>(INT32_MAX),
    ...
    const auto count = [limit](asc::extent_t n, asc::extent_t nrhs,
      ...
      return counts::Driver(n, nrhs, lda, ldb, limit);
    };
    ASC_DENSE_TEST_CHECK(test, count(limit / 3, 1, 2, 1).ok());
    ASC_DENSE_TEST_EQ(test, count(limit / 3 + 1, 0, 1, 1).code(),
                      asc::ErrorCode::kOverflow);
    ASC_DENSE_TEST_EQ(test, count(limit, 0, limit, limit).code(),
                      asc::ErrorCode::kOverflow);
    ...
    ASC_DENSE_TEST_EQ(test, count(2, (limit - 1) / 3 + 1, 2, 3).code(),
                      asc::ErrorCode::kOverflow);
    ASC_DENSE_TEST_CHECK(test, count(2, 0, (limit - 1) / 2, 1).ok());
    ASC_DENSE_TEST_EQ(test, count(2, 0, (limit - 1) / 2 + 1, 1).code(),
                      asc::ErrorCode::kOverflow);
    ...
    ASC_DENSE_TEST_EQ(test, count(1, 1, 1, 0).code(),
                      asc::ErrorCode::kInvalidArgument);
    for (bool complex_symmetric : {false, true}) {
      ASC_DENSE_TEST_EQ(
          test, *counts::Preferred<float>(0, complex_symmetric, limit), 0);
      ASC_DENSE_TEST_EQ(test,
                        *counts::Preferred<float>(1, complex_symmetric, limit),
                        complex_symmetric ? 65 : 2);
      ASC_DENSE_TEST_EQ(test,
                        *counts::Preferred<double>(1, complex_symmetric, limit),
                        complex_symmetric ? 65 : 2);
      ASC_DENSE_TEST_EQ(
          test,
          counts::Preferred<double>(limit / 65 + 1, complex_symmetric, limit)
              .status()
              .code(),
          asc::ErrorCode::kOverflow);
      ASC_DENSE_TEST_EQ(
          test,
          counts::Preferred<float>(limit / 65, complex_symmetric, limit)
              .status()
              .code(),
          asc::ErrorCode::kOverflow);
      const auto n = (asc::extent_t{1} << 24) / 65 + 2;
      ASC_DENSE_TEST_CHECK(test, *counts::Preferred<float>(n, complex_symmetric,
                                                           limit) >= 65 * n);
    }
  }
}
...
int Run(bool hermitian) {
    ...
    for (const auto layout : {base::kColumn, base::kRow}) {
      for (const auto rhs_layout : {base::kColumn, base::kRow}) {
        for (int bad = 0; bad < 52; ++bad) {
          // Symmetric column A and column B need no packing region.
          if (!hermitian && layout == base::kColumn &&
              rhs_layout == base::kColumn &&
  ...
  std::printf(
      "Aasen driver rejection cases=%d protected_queries=32 empty_noncalls=16 "
      "count_checks=58\n",
      cases);
  return test.Finish();
}
