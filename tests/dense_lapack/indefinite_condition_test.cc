...
auto Query(const asc::ReferenceLapackProvider& provider,
  ...
  if constexpr (asc::DenseBlasComplex<T>) {
    if (hermitian) {
      return asc::QueryHeconWorkspace(provider, triangle, a, pivots, norm,
                                      condition);
    }
  }
  return asc::QuerySyconWorkspace(provider, triangle, a, pivots, norm,
                                  condition);
}
...
asc::Status Condition(const asc::ReferenceLapackProvider& provider,
  ...
  if constexpr (asc::DenseBlasComplex<T>) {
    if (hermitian) {
      return asc::Hecon(provider, triangle, a, pivots, norm, condition, plan,
                        workspace, report);
    }
  }
  return asc::Sycon(provider, triangle, a, pivots, norm, condition, plan,
                    workspace, report);
}
