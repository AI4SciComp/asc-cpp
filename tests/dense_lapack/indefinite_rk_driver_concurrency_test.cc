...
struct Sample {
  ...
  auto Extra() { return asc_rk_test::OffDiagonal(e, mode.n); }
  ...
  void Check(TestContext& test, const Sample& before, int nrhs, bool singular,
    ...
    if (stale) {
      ...
      ASC_DENSE_TEST_EQ(test, e, before.e);
      return;
    }
...
void Group(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  const auto active = base::Take(asc_rk_driver_test::Query(
      provider, mode.triangle, hermitian, prototype.Matrix(), prototype.Extra(),
      prototype.Pivots(), prototype.Rhs(2)));
  const auto factor_only = base::Take(asc_rk_driver_test::Query(
      provider, mode.triangle, hermitian, prototype.Matrix(), prototype.Extra(),
    ...
    threads[worker] = std::thread([&, worker] {
      ...
      for (int repeat = 0; repeat < kRepeats; ++repeat) {
        for (const bool ordinary : {false, true}) {
          ...
          const auto status = asc_rk_driver_test::Driver(
              provider, mode.triangle, hermitian, sample.Matrix(),
              sample.Extra(), sample.Pivots(), sample.Rhs(nrhs), plan,
              workspace, report);
          barrier.arrive_and_wait();
...
int Run(bool hermitian) {
  ...
  std::printf(
      "RK driver concurrent groups=%d workers=%d repeats=%d native_calls=%d "
