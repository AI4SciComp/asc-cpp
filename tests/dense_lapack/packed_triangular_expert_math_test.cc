...
struct Direct<float> {
  ...
  static lapack_int PackedCondition(lapack_int n, char norm, char uplo,
    ...
    LAPACK_stpcon(&norm, &uplo, &diag, &n, a, &rcond, work.data(),
                  integer.data(), &info);
    return info;
  }
  static lapack_int Run(bool condition, const float* a, const float* b,
    ...
    if (condition) {
      ...
      LAPACK_stprfs(&uplo, &trans, &diag, &n, &n, a, b, &n, x, &n, &ferr, &berr,
                    work.data(), integer.data(), &info);
    }
    return info;
  }
};
...
struct Direct<double> {
  ...
  static lapack_int PackedCondition(lapack_int n, char norm, char uplo,
    ...
    [[maybe_unused]] std::array<lapack_int, 3> integer{};
    LAPACK_dtpcon(&norm, &uplo, &diag, &n, a, &rcond, work.data(),
                  integer.data(), &info);
    return info;
  }
  static lapack_int Run(bool condition, const double* a, const double* b,
    ...
    if (condition) {
      ...
      LAPACK_dtprfs(&uplo, &trans, &diag, &n, &n, a, b, &n, x, &n, &ferr, &berr,
                    work.data(), integer.data(), &info);
    }
    return info;
  }
...
struct Direct<std::complex<float>> {
  ...
  static lapack_int PackedCondition(lapack_int n, char norm, char uplo,
    ...
    [[maybe_unused]] std::array<float, 3> real{};
    [[maybe_unused]] std::array<lapack_int, 3> integer{};
    LAPACK_ctpcon(&norm, &uplo, &diag, &n, a, &rcond, work.data(), real.data(),
                  &info);
    return info;
  }
  static lapack_int Run(bool condition, const std::complex<float>* a,
    ...
    [[maybe_unused]] std::array<lapack_int, 1> integer{};
    if (condition) {
      LAPACK_ctpcon(&norm, &uplo, &diag, &n, a, &rcond, work.data(),
                    real.data(), &info);
    } else {
      LAPACK_ctprfs(&uplo, &trans, &diag, &n, &n, a, b, &n, x, &n, &ferr, &berr,
                    work.data(), real.data(), &info);
    }
    return info;
  }
};
...
struct Direct<std::complex<double>> {
  ...
  static lapack_int PackedCondition(lapack_int n, char norm, char uplo,
    ...
    [[maybe_unused]] std::array<double, 3> real{};
    [[maybe_unused]] std::array<lapack_int, 3> integer{};
    LAPACK_ztpcon(&norm, &uplo, &diag, &n, a, &rcond, work.data(), real.data(),
                  &info);
    return info;
  }
  static lapack_int Run(bool condition, const std::complex<double>* a,
    ...
    [[maybe_unused]] std::array<lapack_int, 1> integer{};
    if (condition) {
      LAPACK_ztpcon(&norm, &uplo, &diag, &n, a, &rcond, work.data(),
                    real.data(), &info);
    } else {
      LAPACK_ztprfs(&uplo, &trans, &diag, &n, &n, a, b, &n, x, &n, &ferr, &berr,
                    work.data(), real.data(), &info);
    }
    return info;
  }
...
void Case(Checks& checks, const asc::ReferenceLapackProvider& provider,
  ...
  const auto av = Take(asc::DenseBlasPackedMatrixView<const T>::Create(
      a.data(), 1, asc::DenseBlasLayout::kColumnMajor,
  ...
  const auto plan = Take(
      condition ? asc::QueryTpconWorkspace(provider, norm, kUpper, kNonUnit, av,
                                           values.rcond)
                : asc::QueryTprfsWorkspace(provider, kUpper, kNonUnit, kNone,
                                           av, view(b), view(x), fv, bv));
  std::array<T, 3> work{};
  ...
  const auto status =
      condition ? asc::Tpcon(provider, norm, kUpper, kNonUnit, av, values.rcond,
                             plan, workspace, report)
                : asc::Tprfs(provider, kUpper, kNonUnit, kNone, av, view(b),
                             view(x), fv, bv, plan, workspace, report);
  Estimates<Real> raw;
  ...
  std::printf(
      ...
      Direct<T>::kName, condition ? "TPCON" : "TPRFS", label,
...
template <typename T>
std::array<T, 6> OverflowMatrix(asc::extent_t n,
  ...
  for (asc::extent_t major = 0; major < n; ++major) {
    for (asc::extent_t minor = 0; minor < n; ++minor) {
      ...
      if (triangle == kUpper ? i <= j : i >= j) {
        a[cursor++] = T{(i + j) % 2 == 0 ? scale : -scale};
      }
    }
  }
  return a;
...
template <typename T>
void PackedOverflowCase(Checks& checks,
  ...
  const auto before = a;
  const auto view = Take(asc::DenseBlasPackedMatrixView<const T>::Create(
      a.data(), n, layout, {a.data(), sizeof(a), kHost}));
  Real rcond = Real{-1};
  const auto plan = Take(asc::QueryTpconWorkspace(provider, norm, triangle,
                                                  kNonUnit, view, rcond));
  ConditionWorkspace<T> workspace(plan);
  asc::LapackReport report;
  const auto status = asc::Tpcon(provider, norm, triangle, kNonUnit, view,
                                 rcond, plan, workspace.value, report);
  Real raw = Real{-1};
  const auto column =
      OverflowMatrix<T>(n, scale, asc::DenseBlasLayout::kColumnMajor, triangle);
  const lapack_int info = Direct<T>::PackedCondition(
  ...
  checks.Expect(
      ...
      "packed overflow fixture returns native INFO");
  checks.Expect(Same(rcond, raw), "packed overflow fixture exact raw mapping");
  checks.Expect(a == before,
                "packed overflow preserves selected data and tail");
  checks.Expect(status.ok() && std::isfinite(rcond) &&
                ...
                "finite packed triangular matrix has analytic RCOND 1/(2*n)");
  std::printf(
      "%s packed norm n=%lld scale=%.21Lg layout=%d triangle=%d norm=%d "
...
template <typename T>
void PackedOverflow(Checks& checks,
                    const asc::ReferenceLapackProvider& provider) {
  using Real = asc::DenseBlasRealType<T>;
      ...
      for (const auto layout : {asc::DenseBlasLayout::kColumnMajor,
        ...
        for (const auto triangle : {kUpper, asc::DenseBlasTriangle::kLower}) {
          for (const auto norm : {asc::LapackConditionNorm::kOne,
            ...
            PackedOverflowCase<T>(checks, provider, n, scale, layout, triangle,
                                  norm);
          }
        }
      }
...
int Scalar(bool condition) {
  ...
  if (condition) {
    PackedOverflow<T>(checks, provider);
  }
...
int main(int argc, char** argv) {
  ...
  const bool condition = std::strcmp(argv[2], "tpcon") == 0;
  if (!condition && std::strcmp(argv[2], "tprfs") != 0) {
    return 2;
  }
