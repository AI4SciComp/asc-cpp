...
// Reuse the maintained LACPY observation mechanism with distinct DMD pages.
// Only ASC before the actual wrapped GEDMD entry is observed. The pinned
...
thread_local std::array<asc::ConstMemoryView, 7> g_outputs{
...
void ProtectedPreflight(TestContext& test, Page<T>& x, Page<T>& y, bool empty,
  ...
  if (!empty) {
    for (const auto input : {x.Storage(), y.Storage()}) {
      ExpectFault(test, [&] {
  ...
  if (!empty) {
    ...
    y.Protect(PROT_READ | PROT_WRITE);
    for (const auto memory : g_outputs) {
...
void Observe(TestContext& test, const asc::ReferenceLapackProvider& provider,
             asc::LapackDmdOptions options, bool reverse, bool empty) {
  ...
  const asc::extent_t n = empty ? 0 : 1;
  auto xv = x.Matrix(2, n, first);
  auto yv = y.Matrix(2, n, second);
  if (!empty) {
    x.data()[0] = T{1};
    x.data()[1] = T{};
    y.data()[0] = T{2};
  ...
  const asc::LapackDmdBuffers<T> buffers{xv,
                                         yv,
                                         z.Matrix(2, n, first),
                                         b.Matrix(2, n, second),
                                         w.Matrix(n, n, first),
  ...
  residual.data()[0] = -9;
  const auto plan =
      Take(asc::QueryGedmdWorkspace(provider, options, buffers, Real{0}));
  support::Scratch<T> scratch(plan, false);
  asc::index_t rank = -73;
  asc::LapackReport report;
  g_outputs = {z.Storage(),       b.Storage(),     w.Storage(),
               s.Storage(),       eigen.Storage(), singular.Storage(),
               residual.Storage()};
  fault::Reset();
  fault::OnEntry(RestoreAtEntry);
  const auto run = [&](const asc::LapackWorkspacePlan& selected,
                       const asc::LapackWorkspace& workspace) {
    return asc::Gedmd(provider, options, buffers, Real{0}, rank, selected,
                      workspace, report);
  };
  ProtectedPreflight(
      test, x, y, empty, plan, scratch.workspace, report, rank,
      [&] {
        return asc::QueryGedmdWorkspace(provider, options, buffers, Real{0});
      },
      run);
...
void Run(TestContext& test) {
  ...
  for (const auto svd :
    ...
    const std::array modes{
        asc::LapackDmdOptions{asc::LapackDmdScaling::kNone, Vectors::kNone,
                              Extra::kNone, svd, false, -1},
        asc::LapackDmdOptions{asc::LapackDmdScaling::kNone, Vectors::kFactored,
                              Extra::kNone, svd, false, -1},
        asc::LapackDmdOptions{asc::LapackDmdScaling::kConsistentSnapshots,
                              Vectors::kExplicit, Extra::kRefinement, svd, true,
                              -1},
        asc::LapackDmdOptions{asc::LapackDmdScaling::kNone, Vectors::kNone,
                              Extra::kExact, svd, false, -1}};
    for (const auto options : modes) {
      for (const bool reverse : {false, true}) {
        Observe<T>(test, provider, options, reverse, false);
      }
    }
    Observe<T>(test, provider, modes[0], false, true);
  }
}
}  // namespace
int main() {
