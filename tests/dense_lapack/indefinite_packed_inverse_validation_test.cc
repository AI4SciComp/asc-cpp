...
using asc_packed_inverse_test::Inverse;
using asc_packed_inverse_test::Query;
using base::TestContext;

template <typename T>
void AlterExtraWorkspace(int bad, asc::LapackWorkspacePlan& plan,
                         asc::LapackWorkspace& workspace,
                         base::Scratch<T>& scratch, std::array<T, 16>& a) {
  switch (bad) {
    case 29:
...
void Rejection(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  const auto status = base::WithoutAllocation(test, [&] {
    return Inverse(provider, selected, hermitian, matrix, pivots, plan,
                   workspace, report);
  });
...
int Run(bool hermitian) {
  ...
  for (const auto triangle : {base::kUpper, base::kLower}) {
    for (const auto layout : {base::kColumn, base::kRow}) {
      for (int bad = 0; bad < 42; ++bad) {
        std::printf("Packed inverse rejection tri=%d layout=%d fault=%d\n",
  ...
  ASC_DENSE_TEST_EQ(test, cases, 168);
  std::printf("packed inverse rejection cases=%d\n", cases);
  return test.Finish();
}
}  // namespace
