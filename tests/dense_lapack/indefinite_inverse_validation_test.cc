...
#include "../../src/dense/lapack/internal_indefinite_inverse_counts.h"
...
using asc_inverse_test::Inverse;
using asc_inverse_test::Query;
...
void Rejection(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  ASC_DENSE_TEST_EQ(test, plan.regions[base::kScalar].minimum_entries,
                    asc::DenseBlasComplex<T> && !hermitian ? 4 : 2);
...
void Counts(TestContext& test, bool hermitian) {
  ...
  const auto count = [hermitian](asc::extent_t n, asc::extent_t lda) {
    return asc::internal_indefinite_inverse_counts::Inverse(n, lda, hermitian,
                                                            kLimit);
  };
  ASC_DENSE_TEST_CHECK(test, count(kLimit - 1, 1).ok());
  ASC_DENSE_TEST_EQ(test, count(kLimit, 1).code(), asc::ErrorCode::kOverflow);
  ASC_DENSE_TEST_CHECK(test, count(2, kLimit).ok());
  ASC_DENSE_TEST_CHECK(test, count(3, kLimit - 1).ok());
  ASC_DENSE_TEST_EQ(test, count(3, kLimit).ok(), hermitian);
  ASC_DENSE_TEST_EQ(test, count(kLimit - 1, 2).ok(), hermitian);
  ASC_DENSE_TEST_EQ(test, count(-1, 1).code(),
                    asc::ErrorCode::kInvalidArgument);
  ASC_DENSE_TEST_EQ(test, count(1, 0).code(), asc::ErrorCode::kInvalidArgument);
}
template <typename T>
int Run(bool hermitian) {
  ...
  for (const auto triangle : {base::kUpper, base::kLower}) {
    for (const auto layout : {base::kColumn, base::kRow}) {
      for (int bad = 0; bad < 29; ++bad) {
        Rejection<T>(test, provider, hermitian, triangle, layout, bad);
        ++cases;
      }
    }
  }
  Counts(test, hermitian);
  Unread<T>(test, provider, hermitian);
  std::printf("classic inverse rejection cases=%d\n", cases);
  return test.Finish();
}
}  // namespace
