...
#include "../../src/dense/lapack/internal_indefinite_block_inverse_counts.h"
...
using asc_block_inverse_test::Inverse;
using asc_block_inverse_test::Query;
...
void Rejection(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  ASC_DENSE_TEST_EQ(test, plan.regions[base::kScalar].minimum_entries,
                    asc_block_inverse_test::Entries<T>(2, hermitian));
  ...
  const auto original_block = asc_block_inverse_test::g_block_size;
  if (bad == 29) {
    asc_block_inverse_test::g_block_size = -1;
  }
  if (bad == 30) {
  ...
  if (bad == 31) {
    asc_block_inverse_test::g_block_size = original_block == 1 ? 2 : 1;
  }
  if (bad == 32) {
    asc_block_inverse_test::g_block_size = original_block == 0 ? 64 : 0;
  }
  const auto status = base::WithoutAllocation(test, [&] {
    return Inverse(provider, selected, hermitian, matrix, pivots, plan,
                   workspace, report);
  });
  asc_block_inverse_test::g_block_size = original_block;
      ...
      for (const asc::extent_t n : {0, 1}) {
        ...
        const auto invalid = [&] {
          if constexpr (asc::DenseBlasComplex<T>) {
            if (hermitian) {
              return asc::QueryHetri2xWorkspace(provider, triangle, 0, matrix,
                                                pivots);
            }
          }
          return asc::QuerySytri2xWorkspace(provider, triangle, 0, matrix,
                                            pivots);
        }();
...
void Counts(TestContext& test, bool hermitian) {
  namespace counts = asc::internal_indefinite_block_inverse_counts;
  ...
  for (const bool upper : {false, true}) {
    ...
    // Explicit TRI2X has no native INTEGER LWORK product. This LP64 example
...
int Run(bool hermitian) {
  ...
  std::printf("block inverse rejection cases=%d\n", cases);
  return test.Finish();
}
}  // namespace
