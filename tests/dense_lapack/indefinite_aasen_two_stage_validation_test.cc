...
void Rejection(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  auto plan = base::Take(
      aa::Query(provider, tri, he, matrix, band, pivots, band_pivots));
  ...
  const auto status = base::WithoutAllocation(test, [&] {
    return aa::Factor(provider, selected, he, matrix, band, pivots, band_pivots,
                      plan, workspace, report);
  });
  ASC_DENSE_TEST_CHECK(test, !status.ok());
  if ((bad >= 24 && bad <= 26) || bad >= 52) {
  ...
  for (const auto tri : {base::kUpper, base::kLower}) {
    for (const auto layout : {base::kColumn, base::kRow}) {
      for (const asc::extent_t n : {0, 1, 2}) {
        ...
        if (n == 0) {
          asc::LapackReport report;
          const auto status = aa::Factor(provider, tri, he, matrix, band,
                                         pivots, band_pivots, plan, {}, report);
          ASC_DENSE_TEST_CHECK(test, status.ok());
          ASC_DENSE_TEST_CHECK(
              test, !report.called_provider && !report.native_info.has_value());
        }
...
void OriginalLeading(TestContext& test,
  ...
  for (const auto tri : {base::kUpper, base::kLower}) {
    for (const auto layout : {base::kColumn, base::kRow}) {
      ...
      asc::LapackReport report;
      const auto status = base::WithoutAllocation(test, [&] {
        return aa::Factor(provider, tri, he, matrix, band, pivots, band_pivots,
                          plan, workspace, report);
      });
...
int Run(bool he) {
  ...
  std::printf("Two-stage validation rejections=%d\n", cases);
  return test.Finish();
}
}  // namespace
