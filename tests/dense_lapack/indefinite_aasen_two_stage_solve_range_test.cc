...
int Run(bool he, bool fidelity) {
        ...
        for (bool row : {false, true}) {
          for (bool rhs_row : {false, true}) {
            for (int band : {0, 2}) {
              for (int work : {0, 2}) {
                std::printf(
                    "Two-stage solve range kind=%d scale=%La upper=%d row=%d "
                    "rhs_row=%d band=%d work=%d\n",
                    kind, static_cast<long double>(scale), upper, row, rhs_row,
                    band, work);
                // Producer range reconstruction already has exact pinned
                // evidence. Here retain the required consumer X/residual gate.
                aa::Case(
                    test, provider,
  ...
  std::printf("Two-stage Aasen solve range cases=%d\n", cases);
  return test.Finish();
}
}  // namespace
