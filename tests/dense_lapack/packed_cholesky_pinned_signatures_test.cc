...
// Hand-reviewed complete PPTRF signatures, checked separately against GNU
...
static_assert(kMatches<decltype(LAPACK_spptrf_base), Factor<float>>);
static_assert(kMatches<decltype(LAPACK_dpptrf_base), Factor<double>>);
static_assert(
    kMatches<decltype(LAPACK_cpptrf_base), Factor<std::complex<float>>>);
static_assert(
    kMatches<decltype(LAPACK_zpptrf_base), Factor<std::complex<double>>>);
...
int main() {
  ...
  std::printf(
      "4 complete packed Cholesky factor signatures passed; configured "
