...
#include "../../src/dense/lapack/internal_indefinite_aasen_two_stage_driver_counts.h"
...
void CountRegression(base::TestContext& test) {
  namespace counts = asc::internal_indefinite_aasen_two_stage_driver_counts;
  for (const std::int64_t limit :
       {std::int64_t{INT32_MAX}, std::int64_t{INT64_MAX}}) {
    ASC_DENSE_TEST_CHECK(test, counts::Driver(2, 0, 2, 8, 2, limit).ok());
    ASC_DENSE_TEST_CHECK(test, counts::Driver(0, 3, 1, 0, 1, limit).ok());
    ASC_DENSE_TEST_EQ(test, counts::Driver(2, 0, 2, 7, 1, limit).code(),
                      asc::ErrorCode::kShape);
    ASC_DENSE_TEST_EQ(test, counts::Driver(2, 1, 2, 8, 1, limit).code(),
                      asc::ErrorCode::kShape);
    ASC_DENSE_TEST_CHECK(test, counts::Driver(2, 1, 2, 8, 2, limit).ok());
    ASC_DENSE_TEST_EQ(
        test, counts::Driver(limit / 577 + 1, 0, 1, limit, 1, limit).code(),
        asc::ErrorCode::kOverflow);
    ASC_DENSE_TEST_EQ(test, counts::Driver(2, limit, 2, 8, 2, limit).code(),
                      asc::ErrorCode::kOverflow);
    ASC_DENSE_TEST_EQ(
        test, counts::Driver(7, 33, 7, 28, (limit - 1) / 33 + 1, limit).code(),
        asc::ErrorCode::kOverflow);
  }
}
template <typename T>
int Run(bool he, bool fidelity) {
  ...
  std::printf("Two-stage Aasen checked driver cases=%d\n", cases);
  return test.Finish();
}
}  // namespace
