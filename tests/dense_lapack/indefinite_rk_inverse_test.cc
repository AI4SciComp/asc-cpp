...
#include "indefinite_rk_inverse_native.h"
...
using asc_rk_inverse_test::Scratch;
template <typename T>
struct Sample {
  ...
  Sample(int order, bool he, asc::DenseBlasTriangle tri,
    ...
    for (int j = 0; j < n; ++j) {
      for (int i = j; i < n; ++i) {
        ...
        if (order == 3) {
          // A distant largest off-diagonal entry forces an RK 2x2
          // pivot with a nontrivial interchange in either triangle.
...
// It uses row partial pivoting, not stored RK factors or a provider solve.
...
template <typename T>
void NativeFidelity(TestContext& test, const Sample<T>& sample,
                    const std::array<T, 5000>& before,
                    const std::array<asc::index_t, 72>& pivots_before,
                    const asc::LapackReport& report) {
  std::array<T, 4489> native{};
  std::array<T, 17000> native_work{};
  std::array<lapack_int, 72> native_pivots{};
  for (int j = 0; j < sample.n; ++j) {
    native_pivots[j] =
        static_cast<lapack_int>(pivots_before[static_cast<std::size_t>(j) + 1]);
    for (int i = 0; i < sample.n; ++i) {
      if (sample.Selected(i, j)) {
        native[j * sample.n + i] = before[sample.Offset(i, j)];
      }
    }
  }
  const char tri = sample.triangle == kUpper ? 'U' : 'L';
  const lapack_int order = sample.n;
  lapack_int info = std::numeric_limits<lapack_int>::min();
  asc_rk_inverse_test::Native(sample.hermitian, &tri, &order, native.data(),
                              &order, sample.extra.data() + 1,
                              native_pivots.data(), native_work.data(), &info);
  ASC_DENSE_TEST_EQ(test, info, report.native_info.value_or(-999));
  for (int j = 0; j < sample.n; ++j) {
    ASC_DENSE_TEST_EQ(test, native_pivots[j],
                      pivots_before[static_cast<std::size_t>(j) + 1]);
    for (int i = 0; i < sample.n; ++i) {
      if (sample.Selected(i, j)) {
        const auto value = sample.a[sample.Offset(i, j)];
        ASC_DENSE_TEST_CHECK(
            test, EqualBytes(&value, &native[j * sample.n + i], sizeof(T)));
      }
    }
  }
}
...
template <typename T>
void SingularOutput(TestContext& test, const Sample<T>& sample,
                    const std::array<T, 5000>& before) {
  ASC_DENSE_TEST_CHECK(
      test, EqualBytes(sample.a.data(), before.data(), sizeof(before)));
}
template <typename T>
void PoisonExtra(Sample<T>& sample) {
  using Real = asc::DenseBlasRealType<T>;
  for (int i = 0; i < sample.n;) {
    const bool pair = sample.pivots[static_cast<std::size_t>(i) + 1] < 0;
    const int ignored = pair && sample.triangle == kLower ? i + 1 : i;
    sample.extra[static_cast<std::size_t>(ignored) + 1] =
        Value<T>(std::numeric_limits<Real>::quiet_NaN(), 19);
    i += pair ? 2 : 1;
  }
}

template <typename T>
void PivotInterchange(TestContext& test, const Sample<T>& sample,
                      bool singular) {
  if (sample.n == 3 && !singular) {
    ...
    ASC_DENSE_TEST_CHECK(
        test,
        sample.pivots[first] != -static_cast<asc::index_t>(first) ||
            sample.pivots[first + 1] != -static_cast<asc::index_t>(first + 1));
  }
}

template <typename T>
void Case(TestContext& test, const asc::ReferenceLapackProvider& provider,
          Sample<T> sample, bool blocked, bool singular, bool mathematical) {
  auto a = sample.View();
  auto ipiv = Pivots(sample.pivots, sample.n);
  auto extra = asc_rk_test::OffDiagonal(sample.extra, sample.n);
  const auto factor_plan = Take(asc_rk_test::QueryFactor(
      provider, sample.triangle, sample.hermitian, blocked, a, extra, ipiv));
  ...
  PivotInterchange(test, sample, singular);
  PoisonExtra(sample);
  const auto immutable_extra =
      asc_rk_inverse_test::OffDiagonal(sample.extra, sample.n);
  const auto extra_before = sample.extra;
  ...
  const auto plan = Take(WithoutAllocation(test, [&] {
    return asc_rk_inverse_test::Query(
        provider, sample.triangle, sample.hermitian, a, immutable_extra, raw);
  }));
  const auto scalar_role =
      static_cast<std::size_t>(asc::LapackWorkspaceKind::kScalar);
  const asc::extent_t expected_work =
      asc_rk_inverse_test::Entries<T>(sample.n, sample.hermitian);
  ASC_DENSE_TEST_EQ(test, plan.regions[scalar_role].minimum_entries,
                    expected_work);
  ASC_DENSE_TEST_EQ(test, plan.regions[scalar_role].preferred_entries,
                    expected_work);
  Scratch<T> scratch;
  const auto workspace = scratch.Workspace(plan);
  asc::LapackReport report;
  const auto status = WithoutAllocation(test, [&] {
    return asc_rk_inverse_test::Inverse(provider, sample.triangle,
                                        sample.hermitian, a, immutable_extra,
  ...
  if (singular) {
    ...
    ASC_DENSE_TEST_EQ(test, report.diagnostic_index.value_or(-999),
                      sample.n - 1);
    SingularOutput(test, sample, before);
  } else {
  ...
  if (!mathematical && sample.n != 0) {
    NativeFidelity(test, sample, before, pivots_before, report);
  }
  ASC_DENSE_TEST_CHECK(test,
                       EqualBytes(sample.extra.data(), extra_before.data(),
                                  sizeof(extra_before)));
...
int Run(bool hermitian, bool mathematical) {
  ...
  std::printf("RK inverse cases=%d\n", cases);
  return test.Finish();
}
}  // namespace
int main(int argc, char** argv) {
  const asc_lapack_test::NormalReturnGuard return_guard;
  if (argc != 4 || !asc_rk_inverse_test::Select(argv[2])) {
    return 2;
  }
