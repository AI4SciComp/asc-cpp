#ifndef ASC_TESTS_DENSE_LAPACK_INDEFINITE_RK_INVERSE_TEST_SUPPORT_H_
#define ASC_TESTS_DENSE_LAPACK_INDEFINITE_RK_INVERSE_TEST_SUPPORT_H_
#include <array>
#include <cstddef>
#include <string_view>
#include <type_traits>
  ...
  if constexpr (asc::DenseBlasComplex<T>) {
    if (hermitian) {
      return g_block_size == 0
                 ? asc::QueryHetri3Workspace(provider, triangle, factors,
                                             off_diagonal, pivots)
                 : asc::QueryHetri3xWorkspace(provider, triangle, g_block_size,
                                              factors, off_diagonal, pivots);
    }
  }
  return g_block_size == 0
             ? asc::QuerySytri3Workspace(provider, triangle, factors,
                                         off_diagonal, pivots)
...
asc::Status Inverse(const asc::ReferenceLapackProvider& provider,
  ...
  if constexpr (asc::DenseBlasComplex<T>) {
    if (hermitian) {
      return g_block_size == 0
                 ? asc::Hetri3(provider, triangle, factors, off_diagonal,
                               pivots, plan, workspace, report)
                 : asc::Hetri3x(provider, triangle, g_block_size, factors,
                                off_diagonal, pivots, plan, workspace, report);
    }
  }
  return g_block_size == 0
             ? asc::Sytri3(provider, triangle, factors, off_diagonal, pivots,
                           plan, workspace, report)
             : asc::Sytri3x(provider, triangle, g_block_size, factors,
                            off_diagonal, pivots, plan, workspace, report);
}
...
using asc_rk_solve_test::OffDiagonal;
template <typename T>
struct Scratch {
...
#endif  // ASC_TESTS_DENSE_LAPACK_INDEFINITE_RK_INVERSE_TEST_SUPPORT_H_
