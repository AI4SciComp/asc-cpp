...
bool Output(const installed_internal::Matrix<T, N, N>& a,
  ...
  if (singular) {
    ...
    if constexpr (N >= 2) {
      if (g_block_size != 0 ||
          N > static_cast<std::size_t>(Block<T>(hermitian))) {
    ...
    return ok && a.data == expected.data;
  }
...
bool Check(const asc::ReferenceLapackProvider& provider, bool hermitian,
  ...
  if (!ok) {
    std::fprintf(stderr,
                 "installed block inverse failure N=%zu he=%d tri=%d layout=%d "
                 "origin=%d singular=%d\n",
...
int main() {
  ...
  std::printf("installed block inverse cases=%d passed=%d\n", cases,
              static_cast<int>(ok));
  return ok ? 0 : 1;
}
