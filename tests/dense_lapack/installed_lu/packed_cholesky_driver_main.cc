...
struct Checks {
  ...
  void Expect(bool value) {
    ...
    if (!value) {
      if (failures < 20) {
        std::fprintf(stderr, "Packed Cholesky driver check failed at %zu\n",
                     checks);
      }
      ++failures;
    }
...
struct Fixture {
  ...
  explicit Fixture(const Profile& p, int exponent) {
    ...
    Slots(p, [&](std::size_t slot, auto i, auto j) {
      auto value = Matrix<T>(p, i, j) * scale;
      if constexpr (asc::DenseBlasComplex<T>) {
        if (i == j && p.poison_diagonal) {
          value.imag(std::numeric_limits<long double>::quiet_NaN());
        }
      }
      factors[slot + 1] = Narrow<T>(value);
    });
  }
...
template <typename T>
void FactorEquation(Checks& checks, const Fixture<T>& sample, const Profile& p,
                    int exponent) {
  using Real = asc::DenseBlasRealType<T>;
  ...
  Slots(p, [&](std::size_t slot, auto i, auto j) {
    const auto value =
        installed_internal::Widen(sample.factors[slot + 1]) / scale;
    factor[i][j] = value;
    const auto expected =
        p.triangle == kUpper ? std::conj(Lower<T>(j, i)) : Lower<T>(i, j);
    checks.Expect(std::isfinite(value.real()) && std::isfinite(value.imag()));
    checks.Expect(std::abs(value - expected) <= tolerance * (p.n + 2));
  });
  long double residual = 0;
  long double norm = 0;
  for (asc::extent_t i = 0; i < p.n; ++i) {
    checks.Expect(factor[i][i].real() > 0 && factor[i][i].imag() == 0);
    long double residual_row = 0;
    long double norm_row = 0;
    for (asc::extent_t j = 0; j < p.n; ++j) {
      Wide value{};
      for (asc::extent_t k = 0; k < p.n; ++k) {
        value += p.triangle == kUpper ? std::conj(factor[k][i]) * factor[k][j]
                                      : factor[i][k] * std::conj(factor[j][k]);
      }
      const auto expected = Matrix<T>(p, i, j);
      const auto difference = std::abs(value - expected);
      checks.Expect(std::isfinite(difference));
      residual_row += difference;
      norm_row += std::abs(expected);
    }
    residual = std::max(residual, residual_row);
    norm = std::max(norm, norm_row);
  }
  checks.Expect(norm == 0 ? residual == 0 : residual / norm <= tolerance);
}

template <typename T>
void Guards(Checks& checks, const Fixture<T>& sample, const Profile& p,
            asc::extent_t packing_entries) {
  checks.Expect(sample.factors.front() == T{-91});
  const auto count = static_cast<std::size_t>(p.n * (p.n + 1) / 2);
  checks.Expect(std::all_of(sample.factors.begin() + 1 + count,
                            sample.factors.end(),
                            [](T value) { return value == T{-91}; }));
  checks.Expect(sample.packing.front() == T{-97});
  checks.Expect(std::all_of(sample.packing.begin() + 1 + packing_entries,
                            sample.packing.end(),
...
void Run(Checks& checks, const asc::ReferenceLapackProvider& provider,
  ...
  const auto plan = Take(
      asc::QueryPpsvWorkspace(provider, p.triangle, sample.A(p), sample.B(p)));
  const bool active = p.n != 0;
  ...
  for (int repeat = 0; repeat < 2; ++repeat) {
    ...
    FactorEquation(checks, sample, p, exponent);
    ...
    Guards(checks, sample, p, expected_entries);
    ++checks.workflows;
  }
}
...
void Nonpositive(Checks& checks, const asc::ReferenceLapackProvider& provider,
                 const Profile& p, asc::extent_t pivot, int exponent) {
  using Real = asc::DenseBlasRealType<T>;
  Fixture<T> sample(p, exponent);
  const auto scale = std::ldexp(1.0L, exponent);
  const auto factor_scale = std::ldexp(1.0L, exponent / 2);
  Slots(p, [&](std::size_t slot, auto i, auto j) {
    Wide value{};
    if (i == j) {
      value.real((i == pivot ? -1 : (i + 2) * (i + 2)) * scale);
      if constexpr (asc::DenseBlasComplex<T>) {
        value.imag(static_cast<long double>(17 + i));
      }
    }
    sample.factors[slot + 1] = Narrow<T>(value);
  });
  auto expected = sample.factors;
  Slots(p, [&](std::size_t slot, auto i, auto j) {
    if (i == j) {
      if (i < pivot) {
        expected[slot + 1] = T{static_cast<Real>((i + 2) * factor_scale)};
      } else if (i == pivot) {
        expected[slot + 1] = T{static_cast<Real>(-scale)};
      } else if (p.triangle != kUpper && pivot > 0) {
        expected[slot + 1] = T{static_cast<Real>((i + 2) * (i + 2) * scale)};
      }
    }
  });
  sample.RightHandSide(p, exponent, 0);
  const auto original_rhs = sample.rhs;
  const auto plan = Take(
      asc::QueryPpsvWorkspace(provider, p.triangle, sample.A(p), sample.B(p)));
  const auto workspace = sample.Workspace(plan);
  asc::LapackReport report;
  const auto status = asc::Ppsv(provider, p.triangle, sample.A(p), sample.B(p),
  ...
  checks.Expect(sample.factors == expected);
  checks.Expect(sample.rhs == original_rhs);
  Guards(checks, sample, p, plan.regions[kPacking].minimum_entries);
  ++checks.workflows;
}
      ...
      for (auto triangle : {kUpper, asc::DenseBlasTriangle::kLower}) {
        for (auto a_layout : {kColumn, kRow}) {
          for (auto b_layout : {kColumn, kRow}) {
            for (const bool poison_diagonal : {false, true}) {
              if (poison_diagonal && !asc::DenseBlasComplex<T>) {
                continue;
              }
              const Profile p{n,        nrhs,     triangle,
                              a_layout, b_layout, poison_diagonal};
              for (int exponent :
                   {-40, -2, 0, 20, 40,
                    std::numeric_limits<Real>::min_exponent - 1,
                    std::numeric_limits<Real>::max_exponent - 10}) {
                Run<T>(checks, provider, p, exponent);
                if (!poison_diagonal && n > 0) {
                  Nonpositive<T>(checks, provider, p, 0, exponent);
                  if (n > 1) {
                    Nonpositive<T>(checks, provider, p, n - 1, exponent);
                  }
                }
              }
            }
...
int main() {
  ...
  checks.Expect(checks.workflows == 14784);
  std::printf(
      "Packed Cholesky driver public: %zu workflows, %zu checks, %zu "
      "failures\n",
