...
template <typename T>
auto QueryDriver(const asc::ReferenceLapackProvider& provider,
  ...
  if constexpr (asc::DenseBlasComplex<T>) {
    if (he) {
      return asc::QueryHesvAaWorkspace(provider, triangle, matrix, pivots, rhs);
    }
  }
  return asc::QuerySysvAaWorkspace(provider, triangle, matrix, pivots, rhs);
}
template <typename T>
asc::Status Driver(const asc::ReferenceLapackProvider& provider,
  ...
  if constexpr (asc::DenseBlasComplex<T>) {
    if (he) {
      return asc::HesvAa(provider, triangle, matrix, pivots, rhs, plan,
                         workspace, report);
    }
  }
  return asc::SysvAa(provider, triangle, matrix, pivots, rhs, plan, workspace,
                     report);
}
template <typename T>
struct Scratch {
...
template <typename T, std::size_t N, std::size_t Rhs>
bool FactorGuards(const installed_internal::Matrix<T, N, N>& a,
                  const installed_internal::Matrix<T, N, Rhs>& b,
                  const std::array<T, (N + 1) * (N + 1)>& original_a,
                  const std::array<T, (N + 1) * (Rhs + 1)>& before_b,
                  const std::array<asc::index_t, N + 2>& pivots,
                  asc::DenseBlasTriangle tri) {
  bool pass = Check(a.PaddingEquals(original_a) && b.PaddingEquals(before_b) &&
                        pivots.front() == -67 && pivots.back() == -67,
                    "driver array guards");
  for (std::size_t j = 0; j < N; ++j) {
    pass = Check(pivots[j + 1] >= static_cast<asc::index_t>(j + 1) &&
                     pivots[j + 1] <= static_cast<asc::index_t>(N),
                 "positive Aasen pivot range") &&
           pass;
    for (std::size_t i = 0; i < N; ++i) {
      if (tri == kUpper ? i > j : i < j) {
        pass = Check(EqualBytes(a.At(i, j), original_a[a.Offset(i, j)]),
                     "unselected driver triangle") &&
               pass;
      }
    }
  }
  if constexpr (N != 0) {
    pass = Check(pivots[1] == 1, "first Aasen pivot") && pass;
  }
  return pass;
}
template <typename T, std::size_t N, std::size_t Rhs>
bool Case(const asc::ReferenceLapackProvider& provider, bool he,
  ...
  const auto fp =
      Take(QueryDriver(provider, tri, he, a.View(), mutable_p, b.View()));
  Scratch<T> factor_scratch(fp, mode, N);
  asc::LapackReport report;
  bool pass = Check(Driver(provider, tri, he, a.View(), mutable_p, b.View(), fp,
  ...
  pass = Check(report.called_provider == (N != 0) &&
               ...
               "driver report including zero RHS factorization") &&
         pass;
  pass = VerifySolution(b, before_b, full, solution) && pass;
  pass = FactorGuards(a, b, original_a, before_b, pivots, tri) && pass;
  b.data = before_b;
...
bool Run(const asc::ReferenceLapackProvider& provider, bool he) {
  ...
  std::printf("Installed AA driver and factor-reuse cases=%d pass=%d\n", cases,
              pass);
  return pass;
}
}  // namespace
