...
class Checks {
 public:
  void Expect(bool value, const char* message) {
    ...
    if (!value) {
      if (failures_ < 30) {
        std::fprintf(stderr, "Triangular band expert: %s\n", message);
      }
      ++failures_;
    }
  }
  ...
  [[nodiscard]] int Finish() const {
    std::printf(
        "Triangular band expert: %zu workflows, %zu checks, %zu failures\n",
...
struct BandMatrix {
  ...
  asc::extent_t order;
  asc::extent_t bandwidth;
  asc::extent_t leading;
  asc::DenseBlasLayout layout;
  asc::DenseBlasTriangle triangle;
  BandMatrix(asc::extent_t n, asc::DenseBlasLayout selected,
             asc::DenseBlasTriangle selected_triangle, asc::extent_t kd)
      : order(n),
        bandwidth(kd),
        leading(kd + 4),
        layout(selected),
    ...
    data.fill(Value<T>(std::numeric_limits<double>::quiet_NaN()));
    // Decode every physical band slot into logical coordinates, independent
    // of the adapter's logical-to-physical offset calculation.
    for (asc::extent_t major = 0; major < n; ++major) {
      for (asc::extent_t slot = 0; slot <= kd; ++slot) {
        asc::extent_t minor = major + slot;
        if ((layout == kColumn && triangle == kUpper) ||
            (layout == kRow && triangle == kLower)) {
          minor = major + slot - kd;
        }
        if (minor >= 0 && minor < n) {
          const auto i = layout == kRow ? major : minor;
          const auto j = layout == kRow ? minor : major;
          offsets[static_cast<std::size_t>(i * n + j)] =
              static_cast<std::size_t>(1 + major * leading + slot);
        }
      }
    }
  }
  ...
  auto View() {
    return Take(asc::LapackTriangularBandView<const T>::Create(
        data.data() + 1, order, bandwidth, triangle, layout, leading,
        {data.data(), sizeof(data), kHost}));
  }
};
...
template <typename T>
BandMatrix<T> Triangle(asc::extent_t n, asc::DenseBlasLayout layout,
                       asc::DenseBlasTriangle triangle,
                       asc::DenseBlasDiagonal diagonal, int exponent, bool band,
                       asc::extent_t kd) {
  BandMatrix<T> a(n, layout, triangle, kd);
  for (asc::extent_t j = 0; j < n; ++j) {
    for (asc::extent_t i = 0; i < n; ++i) {
      if ((triangle == kUpper ? i <= j : i >= j) &&
          (i != j || diagonal == kNonUnit) && std::abs(i - j) <= kd) {
        a.At(i, j) =
            Coefficient<T>(i, j, triangle, diagonal, exponent, band, kd);
      }
    }
  }
  return a;
...
void Condition(Checks& checks, const asc::ReferenceLapackProvider& provider,
               ...
               int exponent, asc::LapackConditionNorm norm, asc::extent_t kd) {
  using Real = asc::DenseBlasRealType<T>;
  auto a = Triangle<T>(n, layout, triangle, diagonal, exponent, false, kd);
  const auto before = a.data;
  Real rcond = Real{-91};
  const auto plan =
      Take(asc::QueryTbconWorkspace(provider, norm, diagonal, a.View(), rcond));
  ...
  checks.Expect(plan.regions[kLayout].minimum_entries ==
                ...
                "exact band condition layout capacity");
  Workspace<T> work(plan);
  asc::LapackReport report;
  const auto status = asc::Tbcon(provider, norm, diagonal, a.View(), rcond,
  ...
  const auto stale = asc::Tbcon(provider, other, diagonal, a.View(), rcond,
                                plan, work.value, report);
  ...
  for (asc::extent_t j = 0; j < nrhs; ++j) {
    ...
    for (asc::extent_t i = 0; i < n; ++i) {
      ...
      for (asc::extent_t k = 0; k < n; ++k) {
        T coefficient =
            operation == kNone
                ? Coefficient<T>(i, k, triangle, diagonal, exponent, true, kd)
                : Coefficient<T>(k, i, triangle, diagonal, exponent, true, kd);
...
void Errors(Checks& checks, const asc::ReferenceLapackProvider& provider,
            ...
            asc::DenseBlasTriangle triangle, asc::DenseBlasDiagonal diagonal,
            asc::DenseBlasTranspose operation, int exponent, asc::extent_t kd) {
  using Real = asc::DenseBlasRealType<T>;
  auto a = Triangle<T>(n, al, triangle, diagonal, exponent, true, kd);
  ...
  const auto plan = Take(asc::QueryTbrfsWorkspace(
      provider, diagonal, operation, a.View(), b.View(), x.View(), fv, bv));
  ...
  checks.Expect(plan.regions[kLayout].minimum_entries == packing,
                "exact independently laid out band A/full B/X capacities");
  Workspace<T> work(plan);
  asc::LapackReport report;
  const auto status =
      asc::Tbrfs(provider, diagonal, operation, a.View(), b.View(), x.View(),
                 fv, bv, plan, work.value, report);
  ...
  checks.Expect(Bytes(a.data, before_a) && Bytes(b.data, before_b) &&
                ...
                "TBRFS preserves all A/B/X bytes including ignored operands");
  work.Guards(checks, plan);
  checks.Workflow();
}
...
void Scalar(Checks& checks, const asc::ReferenceLapackProvider& provider) {
  for (const asc::extent_t n : {0, 1, 2, 5}) {
    for (const auto kd : {asc::extent_t{0}, asc::extent_t{1},
                          std::max<asc::extent_t>(0, n - 1), n, n + 2}) {
                  ...
                  for (const auto operation :
                    ...
                    for (const asc::extent_t nrhs : {0, 3}) {
                      Errors<T>(checks, provider, n, nrhs, al, bl, xl, triangle,
                                diagonal, operation, exponent, kd);
                    }
                  }
                }
              }
