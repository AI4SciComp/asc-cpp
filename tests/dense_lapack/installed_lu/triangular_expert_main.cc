...
class Checks {
 public:
  void Expect(bool value, const char* message) {
    ...
    if (!value) {
      if (failures_ < 30) {
        std::fprintf(stderr, "Triangular expert: %s\n", message);
      }
      ++failures_;
    }
  }
  ...
  [[nodiscard]] int Finish() const {
    std::printf("Triangular expert: %zu workflows, %zu checks, %zu failures\n",
...
template <typename T>
Matrix<T> Triangle(asc::extent_t n, asc::DenseBlasLayout layout,
                   asc::DenseBlasTriangle triangle,
                   asc::DenseBlasDiagonal diagonal, int exponent, bool band) {
  Matrix<T> a(n, n, layout);
...
void Condition(Checks& checks, const asc::ReferenceLapackProvider& provider,
  ...
  Real rcond = Real{-91};
  const auto plan = Take(asc::QueryTrconWorkspace(provider, norm, triangle,
  ...
  const auto status = asc::Trcon(provider, norm, triangle, diagonal, a.View(),
  ...
  const auto stale = asc::Trcon(provider, other, triangle, diagonal, a.View(),
                                rcond, plan, work.value, report);
...
void Errors(Checks& checks, const asc::ReferenceLapackProvider& provider,
  ...
  const auto plan =
      Take(asc::QueryTrrfsWorkspace(provider, triangle, diagonal, operation,
  ...
  asc::LapackReport report;
  const auto status =
      asc::Trrfs(provider, triangle, diagonal, operation, a.View(), b.View(),
  ...
  checks.Expect(Bytes(a.data, before_a) && Bytes(b.data, before_b) &&
                ...
                "TRRFS preserves all A/B/X bytes including ignored operands");
  work.Guards(checks, plan);
  checks.Workflow();
}
