...
bool SolveCases(const asc::ReferenceLapackProvider& provider, bool hermitian,
  ...
  for (const auto rhs_layout : {kColumn, kRow}) {
    ...
    auto query = [&] {
      if constexpr (asc::DenseBlasComplex<T>) {
        if (hermitian) {
          return asc::QueryHetrsWorkspace(provider, factor, b.View());
        }
      }
      return asc::QuerySytrsWorkspace(provider, factor, b.View());
    };
    const auto solve_plan = Take(query());
    auto solve = [&] {
      if constexpr (asc::DenseBlasComplex<T>) {
        if (hermitian) {
          return asc::Hetrs(provider, factor, b.View(), solve_plan,
                            scratch.workspace, report);
        }
      }
      return asc::Sytrs(provider, factor, b.View(), solve_plan,
                        scratch.workspace, report);
    };
...
bool Case(const asc::ReferenceLapackProvider& provider, bool hermitian,
  ...
  if constexpr (N == 2) {
    const asc::index_t encoded = triangle == kUpper ? -1 : -2;
    if (pivots[1] != encoded || pivots[2] != encoded || a.At(0, 0) != T{} ||
        a.At(1, 1) != T{}) {
      return false;
    }
