...
auto Query(const asc::ReferenceLapackProvider& provider,
  ...
  if constexpr (asc::DenseBlasComplex<T>) {
    if (hermitian) {
      return asc::QueryHetrs3Workspace(provider, triangle, factors, extra,
                                       pivots, rhs);
    }
  }
  return asc::QuerySytrs3Workspace(provider, triangle, factors, extra, pivots,
                                   rhs);
}
template <typename T>
asc::Status Solve(const asc::ReferenceLapackProvider& provider,
  ...
  if constexpr (asc::DenseBlasComplex<T>) {
    if (hermitian) {
      return asc::Hetrs3(provider, triangle, factors, extra, pivots, rhs, plan,
                         workspace, report);
    }
  }
  return asc::Sytrs3(provider, triangle, factors, extra, pivots, rhs, plan,
                     workspace, report);
}
template <typename T>
...
template <typename T, std::size_t N>
T BaseCoefficient(std::size_t i, std::size_t j, bool hermitian, bool singular,
                  asc::DenseBlasTriangle triangle) {
  if constexpr (N == 3) {
    if (i == j) {
      return T{};
    }
    const auto low = i < j ? i : j;
    const auto high = i < j ? j : i;
    const bool strong =
        triangle == asc::DenseBlasTriangle::kUpper ? high == 1 : low == 1;
    double magnitude = 1;
    if (strong) {
      magnitude = 4;
    } else if (high - low == 2) {
    ...
    return installed_internal::Value<T>(magnitude,
                                        hermitian && i < j ? -0.125 : 0.125);
  }
  if constexpr (N == 1) {
...
T Coefficient(std::size_t i, std::size_t j, bool hermitian, bool singular,
              asc::DenseBlasTriangle triangle, int exponent) {
  const auto scale = std::ldexp(asc::DenseBlasRealType<T>{1}, exponent);
  return BaseCoefficient<T, N>(i, j, hermitian, singular, triangle) * scale;
}
...
void FillRhs(installed_internal::Matrix<T, N, R>& rhs, bool hermitian,
             bool singular, asc::DenseBlasTriangle triangle, int exponent) {
  rhs.data.fill(T{-43});
  for (std::size_t j = 0; j < R; ++j) {
    for (std::size_t i = 0; i < N; ++i) {
      installed_internal::Wide sum{};
      for (std::size_t k = 0; k < N; ++k) {
        sum += installed_internal::Widen(Coefficient<T, N>(
                   i, k, hermitian, singular, triangle, exponent)) *
...
bool Output(const installed_internal::Matrix<T, N, R>& rhs,
  ...
  for (std::size_t j = 0; j < R; ++j) {
    for (std::size_t i = 0; i < N; ++i) {
      ...
      for (std::size_t k = 0; k < N; ++k) {
        const auto term = installed_internal::Widen(Coefficient<T, N>(
                              i, k, hermitian, false, triangle, exponent)) *
...
template <typename T, std::size_t N>
void PoisonIgnoredExtra(std::array<T, N + 2>& e,
                        const std::array<asc::index_t, N + 2>& p,
                        asc::DenseBlasTriangle triangle) {
  for (std::size_t i = 0; i < N;) {
    const bool paired = p[i + 1] < 0;
    const auto ignored =
        paired && triangle == asc::DenseBlasTriangle::kLower ? i + 1 : i;
...
bool Check(const asc::ReferenceLapackProvider& provider, bool hermitian,
  ...
  for (std::size_t j = 0; j < N; ++j) {
    for (std::size_t i = 0; i < N; ++i) {
      if (triangle == asc::DenseBlasTriangle::kUpper ? i <= j : i >= j) {
        a.At(i, j) =
            Coefficient<T, N>(i, j, hermitian, singular, triangle, exponent);
      }
    }
  }
  installed_internal::Matrix<T, N, R> rhs{{}, rhs_layout};
  FillRhs(rhs, hermitian, singular, triangle, exponent);
  ...
  PoisonIgnoredExtra<T, N>(e, p, triangle);
  const auto old_e = e;
  const asc::DenseBlasVectorView<const T> extra(extra_output);
  const auto before = a;
  const auto old_p = p;
  ...
  Scratch<T> scratch;
  asc::LapackReport report;
  const auto status = Solve(provider, triangle, hermitian, a.ConstView(), extra,
                            raw, rhs.View(), plan, scratch.workspace, report);
  ...
  const auto packing = (layout == installed_internal::kRow ? N * N : 0) +
                       (rhs_layout == installed_internal::kRow ? N * R : 0);
  ok = plan.regions[kScalar].minimum_entries == 0 &&
       plan.regions[kInteger].minimum_entries == (kActive ? N : 0) &&
       plan.regions[kPacking].minimum_entries == (kActive ? packing : 0) && ok;
  ...
  ok = EqualBytes(e.data(), old_e.data(), sizeof(e)) && ok;
  ok = report.factor_family == asc::LapackFactorFamily::kRook && ok;
  ok = Output(rhs, before_rhs, hermitian, singular, triangle, exponent) && ok;
  if (!ok) {
    std::fprintf(stderr,
                 "installed RK solve failure N=%zu NRHS=%zu he=%d tri=%d "
      ...
      for (const auto bl :
        ...
        for (const int origin : {0, 1}) {
          ...
          ok = Check<T, 67, R>(provider, hermitian, triangle, al, bl, origin,
                               true) &&
               ok;
          ok = Check<T, 3, R>(provider, hermitian, triangle, al, bl, origin,
                              false) &&
               ok;
          constexpr int kExponent =
              sizeof(asc::DenseBlasRealType<T>) == sizeof(float) ? 100 : 800;
          for (const int exponent : {-kExponent, kExponent}) {
            ok = Check<T, 7, R>(provider, hermitian, triangle, al, bl, origin,
                                false, exponent) &&
                 ok;
          }
...
int main() {
  ...
  std::printf("installed RK solve cases=%d passed=%d\n", cases,
              static_cast<int>(ok));
  return ok ? 0 : 1;
}
