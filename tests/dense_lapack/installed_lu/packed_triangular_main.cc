...
struct PackedMatrix {
  ...
  asc::extent_t columns;
  asc::DenseBlasLayout layout;
  explicit PackedMatrix(const Profile& profile)
      : rows(profile.n), columns(profile.n), layout(profile.layout) {
    values.fill(Value<T>(std::numeric_limits<double>::quiet_NaN()));
    // Enumerate physical storage in its documented major order. Do not use
    // the adapter's packed-offset formula as the test's indexing oracle.
    std::size_t cursor = 1;
    for (asc::extent_t major = 0; major < rows; ++major) {
      for (asc::extent_t minor = 0; minor < rows; ++minor) {
...
template <typename T>
PackedMatrix<T> Triangular(const Profile& profile) {
  PackedMatrix<T> a(profile);
...
template <typename T>
void Unchanged(Checks& checks, const PackedMatrix<T>& actual,
               const PackedMatrix<T>& before, const Profile& profile,
               bool whole) {
...
template <typename T>
void CheckInverse(Checks& checks, const PackedMatrix<T>& a,
...
void Inverse(Checks& checks, const asc::ReferenceLapackProvider& provider,
  ...
  const auto before = a;
  const auto plan = Take(asc::QueryTptriWorkspace(provider, profile.triangle,
                                                  profile.diagonal, a.View()));
  Unchanged(checks, a, before, profile, true);
  checks.Expect(
      plan.regions[kLayout].minimum_entries ==
          (profile.layout == kRow ? profile.n * (profile.n + 1) / 2 : 0),
      "exact inverse packing formula");
  Workspace<T> workspace(plan);
  asc::LapackReport report;
  const auto status = asc::Tptri(provider, profile.triangle, profile.diagonal,
...
void Solve(Checks& checks, const asc::ReferenceLapackProvider& provider,
  ...
  const auto plan = Take(asc::QueryTptrsWorkspace(provider, profile.triangle,
  ...
  asc::LapackReport report;
  const auto status =
      asc::Tptrs(provider, profile.triangle, profile.diagonal, operation,
      ...
      for (auto triangle : {kUpper, kLower}) {
        for (auto diagonal : {kUnit, kNonUnit}) {
          for (const int exponent : {-20, 0, 20}) {
            ...
            Inverse<T>(checks, provider, profile);
...
// Dyadic inputs remain exactly representable at the normal/subnormal border.
// The upper exponent leaves eight bits for forming multiple RHS; the lower
// one keeps the analytic inverse finite. These reuse the independent series
// inverse and separately formed RHS/forward-error oracles above.
template <typename T>
    ...
    for (auto layout : {kColumn, kRow}) {
      for (auto triangle : {kUpper, kLower}) {
        for (const int exponent :
          ...
          const Profile profile{n, layout, triangle, kNonUnit, exponent};
          Inverse<T>(checks, provider, profile);
          for (auto rhs_layout : {kColumn, kRow}) {
            for (auto operation :
                 {kNone, asc::DenseBlasTranspose::kTranspose,
