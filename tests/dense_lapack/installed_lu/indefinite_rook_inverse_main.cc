...
asc::Status MakeFactor(const asc::ReferenceLapackProvider& provider,
  ...
  if constexpr (asc::DenseBlasComplex<T>) {
    if (hermitian) {
      const auto plan = Take(asc::QueryHesvRookWorkspace(
          provider, triangle, a.View(), pivots, rhs.View()));
      return asc::HesvRook(provider, triangle, a.View(), pivots, rhs.View(),
                           plan, scratch.workspace, report);
    }
  }
  const auto plan = Take(asc::QuerySysvRookWorkspace(
      provider, triangle, a.View(), pivots, rhs.View()));
  return asc::SysvRook(provider, triangle, a.View(), pivots, rhs.View(), plan,
...
bool Check(const asc::ReferenceLapackProvider& provider, bool hermitian,
  ...
  if (!ok) {
    std::fprintf(stderr,
                 "installed rook inverse failure N=%zu he=%d tri=%d layout=%d "
                 "origin=%d singular=%d\n",
...
int main() {
  ...
  std::printf("installed rook inverse cases=%d passed=%d\n", cases,
              static_cast<int>(ok));
  return ok ? 0 : 1;
}
