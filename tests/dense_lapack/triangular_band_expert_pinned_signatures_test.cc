...
// These hand-reviewed function types describe the eight pinned band
...
using Condition = void(char*, char*, char*, Integer*, Integer*, T*, Integer*,
                       Real*, T*, Extra*, Integer*, std::size_t, std::size_t,
                       std::size_t);
template <typename T, typename Real, typename Extra>
using Errors = void(char*, char*, char*, Integer*, Integer*, Integer*, T*,
                    Integer*, T*, Integer*, T*, Integer*, Real*, Real*, T*,
                    Extra*, Integer*, std::size_t, std::size_t, std::size_t);
static_assert(
    kMatches<decltype(LAPACK_stbcon_base), Condition<float, float, Integer>>);
static_assert(
    kMatches<decltype(LAPACK_stbrfs_base), Errors<float, float, Integer>>);
static_assert(
    kMatches<decltype(LAPACK_dtbcon_base), Condition<double, double, Integer>>);
static_assert(
    kMatches<decltype(LAPACK_dtbrfs_base), Errors<double, double, Integer>>);
static_assert(kMatches<decltype(LAPACK_ctbcon_base),
                       Condition<std::complex<float>, float, float>>);
static_assert(kMatches<decltype(LAPACK_ctbrfs_base),
                       Errors<std::complex<float>, float, float>>);
static_assert(kMatches<decltype(LAPACK_ztbcon_base),
                       Condition<std::complex<double>, double, double>>);
static_assert(kMatches<decltype(LAPACK_ztbrfs_base),
...
int main() {
  ...
  std::printf(
      "8 complete band estimator signatures passed; configured "
