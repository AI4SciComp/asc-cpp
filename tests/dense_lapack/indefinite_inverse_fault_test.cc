...
namespace faults = asc_inverse_fault_test;
...
void Case(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  const auto plan = base::Take(
      asc_inverse_test::Query(provider, triangle, hermitian, matrix, raw));
  base::Scratch<T> scratch;
  const auto workspace = scratch.Workspace(plan);
  asc::LapackReport report;
  faults::SetFault(fault);
  const auto status = base::WithoutAllocation(test, [&] {
    return asc_inverse_test::Inverse(provider, triangle, hermitian, matrix, raw,
...
int Run(bool hermitian) {
  ...
  std::printf("classic inverse fault cases=%d integer_bits=%d\n", cases,
