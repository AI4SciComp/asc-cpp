...
namespace {
using asc_triangular_band_test::Fault;
...
class Checks {
 public:
  void Expect(bool condition, const char* message) {
    ...
    if (!condition) {
      if (failures_ < 30) {
        std::fprintf(stderr, "Triangular band failure: %s\n", message);
      }
      ++failures_;
    }
  }
  ...
  [[nodiscard]] int Finish() const {
    std::printf(
        "Triangular band failure acceptance: %zu profiles, %zu checks, %zu "
        "failures\n",
...
template <typename T>
auto ExpectedName() {
  constexpr std::array<std::string_view, 4> kNames{"stbtrs", "dtbtrs", "ctbtrs",
                                                   "ztbtrs"};
...
struct BandPair {
  ...
  asc::DenseBlasTriangle triangle;
  asc::extent_t kd;
  asc::extent_t order = 2;
  asc::extent_t leading;
  asc::MemorySpace space = kHost;
  explicit BandPair(const Profile& profile)
      : layout(profile.a_layout),
        triangle(profile.triangle),
        kd(profile.kd),
        leading(kd + 2) {}
  T& At(std::size_t i, std::size_t j) {
    const auto stride = static_cast<std::size_t>(leading);
    const auto diagonal = static_cast<std::size_t>(
        (layout == kColumn) == (triangle == kUpper) ? kd : 0);
    if (i == j) {
      return data[i * stride + diagonal];
    }
    // Literal n=2 off-diagonal positions for each selected band encoding.
    return data[diagonal == 0 ? 1 : stride + diagonal - 1];
  }
  auto ConstView(asc::DenseBlasTriangle selected) {
    return Take(asc::LapackTriangularBandView<const T>::Create(
        data.data(), order, kd, selected, layout, leading,
...
struct Fixture {
  ...
  explicit Fixture(const Profile& profile)
    ...
    if (profile.kd != 0 && profile.triangle == kUpper) {
      a.At(0, 1) = Value<T>(0.25, 0.125);
    } else if (profile.kd != 0) {
  ...
  asc::Status Call(const asc::ReferenceLapackProvider& provider,
    ...
    return asc::Tbtrs(provider, profile.diagonal, profile.operation,
                      a.ConstView(profile.triangle), B(profile), plan,
                      workspace, report);
  }
};
...
void Inject(Checks& checks, const asc::ReferenceLapackProvider& provider,
  ...
  asc::LapackReport report;
  asc_triangular_band_test::Arm(routine, fault);
  const auto calls = asc_triangular_band_test::Calls(routine);
  ...
  asc_triangular_band_test::Disarm();
  ...
  checks.Expect(asc_triangular_band_test::Calls(routine) == calls + 1,
  ...
  checks.Expect(report.routine == ExpectedName<T>(),
                "injected report names the exact scalar band routine");
...
void Singular(Checks& checks, const asc::ReferenceLapackProvider& provider,
  ...
  checks.Expect(report.routine == ExpectedName<T>(),
                "singular report names the exact scalar band routine");
  checks.Expect(
...
std::size_t TotalCalls() {
  ...
  for (std::size_t i = 0; i < 4; ++i) {
    total += asc_triangular_band_test::Calls(i);
  }
  return total;
}
...
void StaleIdentity(const asc::ReferenceLapackProvider& provider,
  ...
  const asc::extent_t band_leading =
      profile.a_layout == kColumn ? profile.kd + 2 : profile.kd + 1;
  plan.identity = Take(asc::LapackPlanIdentity::Create(
      plan.identity.routine(), scalar,
      std::array<asc::extent_t, 5>{2, profile.kd, profile.nrhs, band_leading,
                                   leading},
      std::array<std::int64_t, 7>{
          triangle, diagonal, static_cast<std::int64_t>(profile.operation),
          layout, static_cast<std::int64_t>(profile.b_layout), profile.kd + 2,
          3},
      identity));
}
...
template <typename T>
bool ConfigureDimensionRejection(Profile& profile, Fixture<T>& sample,
                                 asc::LapackWorkspacePlan& plan,
                                 Rejection rejection) {
  switch (rejection) {
    case Rejection::kStaleOrder:
      ...
      sample.rhs_rows = 1;
      break;
    case Rejection::kStaleBandwidth:
      sample.a.kd = sample.a.kd == 0 ? 1 : 0;
      break;
    case Rejection::kStaleBandLeading:
      ++sample.a.leading;
      break;
    case Rejection::kStaleRhsLeading:
      ++sample.rhs_leading;
      break;
    case Rejection::kStaleBandLayout:
      sample.a.layout = sample.a.layout == kColumn ? kRow : kColumn;
      break;
    case Rejection::kStaleRhsLayout:
      sample.b.layout = sample.b.layout == kColumn ? kRow : kColumn;
      break;
    case Rejection::kStaleRhsCount:
      profile.nrhs = profile.nrhs == 0 ? 2 : 0;
      break;
...
RejectionExpectation ConfigureRejection(
  ...
  if (ConfigureDimensionRejection(profile, sample, plan, rejection)) {
    return {asc::ErrorCode::kInvalidState, false};
  }
  switch (rejection) {
    ...
    case Rejection::kMatrixPlacement:
      sample.a.space = asc::MemorySpace::kPinnedHost;
      expected = asc::ErrorCode::kMemoryAccess;
      break;
    ...
    case Rejection::kProviderAlias:
      // Main owns a mutable provider; this is a live metadata byte span only.
      workspace.regions[0] = {
...
void Reject(Checks& checks, const asc::ReferenceLapackProvider& provider,
  ...
  const auto old_identity = provider.identity();
  ...
  const auto status = sample.Call(provider, profile, plan, workspace, report);
  checks.Expect(provider.identity() == old_identity,
                "rejection preserves provider identity");
...
void Preflight(Checks& checks, const asc::ReferenceLapackProvider& provider,
               const Profile& profile) {
  for (auto rejection :
       {Rejection::kDiagonal,           Rejection::kPlan,
        ...
        Rejection::kWorkspaceOverlap,   Rejection::kStaleOrder,
        Rejection::kStaleBandwidth,     Rejection::kStaleBandLeading,
        Rejection::kStaleRhsLeading,    Rejection::kStaleBandLayout,
        Rejection::kStaleRhsLayout,     Rejection::kStaleRhsCount,
        Rejection::kByteLimit,          Rejection::kProviderAlias}) {
...
void AliasedSolve(Checks& checks, const asc::ReferenceLapackProvider& provider,
  ...
  const auto calls = TotalCalls();
  const auto query =
      asc::QueryTbtrsWorkspace(provider, profile.diagonal, profile.operation,
                               sample.a.ConstView(profile.triangle), b);
  const auto status = asc::Tbtrs(provider, profile.diagonal, profile.operation,
                                 sample.a.ConstView(profile.triangle), b, plan,
                                 workspace, report);
...
void RealCalls(Checks& checks, const asc::ReferenceLapackProvider& provider,
  ...
  for (int repeat = 0; repeat < 2; ++repeat) {
    ...
    const auto libc_allocations = asc_lapack_test::EndAllocationAudit();
    checks.Expect(report.routine == ExpectedName<T>(),
                  "real report names the exact scalar band routine");
    checks.Expect(asc_test::ProcessAllocationCountMatches(cpp_allocations, 0) &&
                      libc_allocations == 0,
                  "initial/repeated real call has no observed allocation");
    checks.Expect(asc_triangular_band_test::Calls(routine) != 0 &&
                      TotalCalls() == calls + 1,
          ...
          for (auto diagonal : {kUnit, kNonUnit}) {
            for (auto operation :
              ...
              for (asc::extent_t nrhs : {0, 2}) {
                const Profile profile{kd,       triangle, diagonal, operation,
