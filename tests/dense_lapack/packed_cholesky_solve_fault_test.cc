...
namespace {
namespace faults = asc_packed_cholesky_solve_test;
using faults::Fault;
...
struct Checks {
  ...
  void Expect(bool condition, const char* message) {
    ...
    if (!condition) {
      if (failures < 30) {
        std::fprintf(stderr, "Packed Cholesky solve failure: %s\n", message);
      }
      ++failures;
    }
  }
...
template <typename T>
auto ExpectedName() {
  constexpr std::array<std::string_view, 4> kNames{"spptrs", "dpptrs", "cpptrs",
                                                   "zpptrs"};
...
struct Fixture {
  ...
  auto Query(const asc::ReferenceLapackProvider& provider, const Profile& p) {
    return asc::QueryPptrsWorkspace(provider, p.triangle, A(p), B(p));
  }
  ...
  auto Call(const asc::ReferenceLapackProvider& provider, const Profile& p,
    ...
    return asc::Pptrs(provider, p.triangle, A(p), B(p), plan, workspace,
                      report);
  }
};
...
void Inject(Checks& checks, const asc::ReferenceLapackProvider& provider,
  ...
  checks.Expect(status.code() == asc::ErrorCode::kProvider &&
                    report.called_provider && report.native_info == Raw(fault),
                "exact raw INFO and provider defect");
  checks.Expect(
      ...
      "nonzero or absent native INFO never means numerical success");
  checks.Expect(!report.diagnostic_index && !report.factor_family,
                "no invented singularity or factor provenance");
  const bool argument =
  ...
  if (p.b_layout == kColumn) {
    expected_b[1] = T{-37};
  }
  checks.Expect(
      Same(sample.a, before.a) && Same(sample.b, expected_b),
      "factor preservation;row defect withholding;direct RHS write survives");
...
void RealCalls(Checks& checks, const asc::ReferenceLapackProvider& provider,
  ...
  for (int repeat = 0; repeat < 2; ++repeat) {
    ...
    checks.Expect(Same(sample.a, before.a) && Same(sample.b, expected),
                  "const factors, inactive RHS and padding preserved");
    ++checks.profiles;
  }
}
...
void ChangeIdentity(const asc::ReferenceLapackProvider& provider,
  ...
  const asc::extent_t ldb = p.nrhs == 0 || p.b_layout == kRow ? 2 : 3;
...
void Preflight(Checks& checks, const asc::ReferenceLapackProvider& provider,
  ...
  if (p.nrhs != 0 && (p.a_layout == kRow || p.b_layout == kRow)) {
...
void AliasedOperands(Checks& checks,
  ...
  asc_dense_test::AllocationProbe allocations;
  const auto query =
      asc::QueryPptrsWorkspace(provider, p.triangle, sample.A(p), b);
  const auto status =
      asc::Pptrs(provider, p.triangle, sample.A(p), b, plan, workspace, report);
  const auto count = allocations.count();
  checks.Expect(asc_test::ProcessAllocationCountMatches(count, 0),
...
int main() {
  ...
  checks.Expect(checks.profiles == 2752, "all scheduled profiles executed");
  std::printf(
      "Packed Cholesky solve failure: %zu profiles, %zu checks, %zu failures\n",
