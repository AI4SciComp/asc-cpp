...
namespace {
using asc_packed_triangular_expert_test::Fault;
...
struct Checks {
  ...
  void Expect(bool value, const char* message) {
    ...
    if (!value) {
      if (failures < 30) {
        std::fprintf(stderr, "Packed triangular expert fault: %s\n", message);
      }
      ++failures;
    }
  }
  [[nodiscard]] int Finish() const {
    std::printf(
        "Packed triangular expert faults: %zu profiles, %zu checks, %zu "
        "failures; "
...
struct PackedPair {
  ...
  // Both packed orders coincide for a selected two-by-two triangle. The
  // independent public fixture enumerates larger-order permutations.
  T& At(std::size_t i, std::size_t j) { return data[i + j]; }
  auto ConstView() {
    return Take(asc::DenseBlasPackedMatrixView<const T>::Create(
        data.data(), 2, layout, {data.data(), sizeof(data), kHost}));
  }
};
...
void Injected(Checks& checks, std::size_t routine, const Profile& profile,
  ...
  for (const auto fault :
       {Fault::kNone, Fault::kNone, Fault::kNoInfo, Fault::kNegativeInfo,
        ...
        Fault::kNoOutput}) {
    const auto calls = asc_packed_triangular_expert_test::Calls(routine);
    asc_packed_triangular_expert_test::Arm(routine, fault);
    ...
    asc_packed_triangular_expert_test::Disarm();
    checks.Expect(
    ...
    checks.Expect(
        asc_packed_triangular_expert_test::Calls(routine) == calls + 1 &&
...
asc::LapackPlanIdentity ChangedIdentity(const Profile& profile,
  ...
  const auto scalar = change_scalar ? asc::LapackScalarKind::kMixedF64F32
  ...
  if (profile.condition) {
    return Take(asc::LapackPlanIdentity::Create(
        ...
        std::array{static_cast<std::int64_t>(profile.norm), triangle, diagonal,
                   layout},
        provider));
  }
  ...
  return Take(asc::LapackPlanIdentity::Create(
      ...
      std::array<asc::extent_t, 4>{2, 2, ldb, ldx},
      std::array<std::int64_t, 8>{triangle, diagonal,
                                  static_cast<std::int64_t>(profile.operation),
                                  layout, static_cast<std::int64_t>(profile.bl),
                                  static_cast<std::int64_t>(profile.xl), 3, 3},
      provider));
}
...
asc::ErrorCode ConfigureWorkspaceRejection(int rejection,
  ...
  switch (rejection) {
    ...
    case 11:
    case 12:
      plan.identity = ChangedIdentity(profile, plan, rejection == 12);
      return asc::ErrorCode::kInvalidState;
    default:
...
void Rejected(Checks& checks, std::size_t routine, const Profile& profile,
  ...
  using Real = asc::DenseBlasRealType<T>;
  for (int rejection = 0; rejection < 13; ++rejection) {
    ...
    checks.Expect(
        !status.ok() &&
            asc_packed_triangular_expert_test::Calls(routine) == calls,
...
void Run(Checks& checks, const asc::ReferenceLapackProvider& provider,
  ...
  const auto query = [&]() {
    if (profile.condition) {
      return asc::QueryTpconWorkspace(provider, profile.norm, profile.triangle,
                                      profile.diagonal, a.ConstView(), rcond);
    }
    return asc::QueryTprfsWorkspace(
  ...
  const auto call = [&](const asc::LapackWorkspacePlan& supplied,
    ...
    if (profile.condition) {
      return asc::Tpcon(provider, profile.norm, profile.triangle,
                        profile.diagonal, a.ConstView(), rcond, supplied,
                        workspace, report);
    }
    return asc::Tprfs(provider, profile.triangle, profile.diagonal,
