...
// Hand-reviewed complete PPEQU signatures, checked separately against GNU
...
using Equilibrate = void(char*, Integer*, T*, Real*, Real*, Real*, Integer*,
                         std::size_t);
static_assert(
    kMatches<decltype(LAPACK_sppequ_base), Equilibrate<float, float>>);
static_assert(
    kMatches<decltype(LAPACK_dppequ_base), Equilibrate<double, double>>);
static_assert(kMatches<decltype(LAPACK_cppequ_base),
                       Equilibrate<std::complex<float>, float>>);
static_assert(kMatches<decltype(LAPACK_zppequ_base),
                       Equilibrate<std::complex<double>, double>>);
...
int main() {
  ...
  std::printf(
      "4 complete packed Cholesky equilibration signatures passed; configured "
