...
namespace faults = asc_rk_solve_fault_test;
...
void Execute(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  if (nrhs == 0 || sample.n == 0 ||
      (b_layout == support::kRow && !Accepted(fault))) {
    ASC_DENSE_TEST_CHECK(
...
void Prepare(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  support::Scratch<T> scratch;
  const auto plan = Take(asc_rk_test::QueryFactor(
  ...
  asc::LapackReport report;
  ASC_DENSE_TEST_CHECK(
      test, asc_rk_test::Factor(
                provider, sample.triangle, sample.hermitian, blocked,
                sample.View(), asc_rk_test::OffDiagonal(sample.e, sample.n),
                support::Pivots(sample.pivots, sample.n), plan, work, report)
                .ok());
...
int Run(bool hermitian) {
  ...
  for (const auto triangle : {support::kUpper, support::kLower}) {
    for (const auto al : {support::kColumn, support::kRow}) {
      for (bool blocked : {false, true}) {
        for (int order : {0, 1, 2, 3, 67}) {
          Sample<T> sample(order, hermitian, triangle, al, 0, false);
          Prepare(test, provider, sample, blocked);
            ...
            for (int nrhs : {0, 1, 3}) {
              for (const auto fault :
                   {Fault::kPass, Fault::kOmitInfo, Fault::kWrite16BitInfo,
                    ...
                    Fault::kPositiveInfo, Fault::kOutOfRangeInfo,
                    Fault::kChangedPivot}) {
  ...
  std::printf("RK solve fault cases=%d empty_noncalls=%d\n", cases, empty);
  return test.Finish();
}
}  // namespace
