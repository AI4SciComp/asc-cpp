...
namespace {
using asc_block_inverse_test::Scratch;
...
// It uses row partial pivoting, not stored classic factors or a provider solve.
...
void NativeFidelity(TestContext& test, const Sample<T>& sample,
  ...
  asc_block_inverse_test::Native(sample.hermitian, &tri, &order, native.data(),
                                 &order, native_pivots.data(),
                                 native_work.data(), &info);
...
void SingularOutput(TestContext& test, const Sample<T>& sample,
                    const std::array<T, 5000>& before) {
  if (asc_block_inverse_test::g_block_size == 0 &&
      sample.n <= asc_block_inverse_test::Block<T>(sample.hermitian)) {
    ASC_DENSE_TEST_CHECK(
        test, EqualBytes(sample.a.data(), before.data(), sizeof(before)));
  } else {
    for (int i = 0; i < sample.n; ++i) {
      ASC_DENSE_TEST_EQ(test, sample.a[sample.Offset(i, i)],
                        before[sample.Offset(i, i)]);
      if (sample.pivots[static_cast<std::size_t>(i) + 1] < 0 &&
          i + 1 < sample.n &&
          sample.pivots[static_cast<std::size_t>(i) + 2] < 0) {
        const auto off = sample.triangle == kUpper ? sample.Offset(i, i + 1)
                                                   : sample.Offset(i + 1, i);
        ASC_DENSE_TEST_EQ(test, sample.a[off], T{});
        ++i;
      }
    }
  }
...
void Case(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  const auto plan = Take(WithoutAllocation(test, [&] {
    return asc_block_inverse_test::Query(provider, sample.triangle,
  ...
  const asc::extent_t expected_work =
      asc_block_inverse_test::Entries<T>(sample.n, sample.hermitian);
  ...
  const auto status = WithoutAllocation(test, [&] {
    return asc_block_inverse_test::Inverse(provider, sample.triangle,
...
int Run(bool hermitian, bool mathematical) {
  ...
  std::printf("block inverse cases=%d\n", cases);
  return test.Finish();
}
}  // namespace
int main(int argc, char** argv) {
  const asc_lapack_test::NormalReturnGuard return_guard;
  if (argc != 4 || !asc_block_inverse_test::Select(argv[2])) {
    return 2;
  }
