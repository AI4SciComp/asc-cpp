...
// Hand-reviewed complete PPTRI signatures, checked separately against GNU
...
template <typename T>
using Factor = void(char*, Integer*, T*, Integer*, std::size_t);
static_assert(kMatches<decltype(LAPACK_spptri_base), Factor<float>>);
static_assert(kMatches<decltype(LAPACK_dpptri_base), Factor<double>>);
static_assert(
    kMatches<decltype(LAPACK_cpptri_base), Factor<std::complex<float>>>);
static_assert(
    kMatches<decltype(LAPACK_zpptri_base), Factor<std::complex<double>>>);
...
int main() {
  ...
  std::printf(
      "4 complete packed Cholesky inverse signatures passed; configured "
