...
void Group(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  const auto active = base::Take(asc_block_inverse_test::Query(
      provider, mode.triangle, hermitian, ordinary.Matrix(mode.n),
      base::Raw(ordinary.p, mode.n)));
  const auto empty_plan = base::Take(asc_block_inverse_test::Query(
      provider, mode.triangle, hermitian, ordinary.Matrix(0),
      base::Raw(ordinary.p, 0)));
  Sample<T> serial_partial = singular;
  {
    asc_block_inverse_test::Scratch<T> scratch;
    const auto workspace = scratch.Workspace(active);
    asc::LapackReport report;
    const auto status = asc_block_inverse_test::Inverse(
        provider, mode.triangle, hermitian, serial_partial.Matrix(mode.n),
        base::Raw(singular.p, mode.n), active, workspace, report);
    ASC_DENSE_TEST_EQ(test, status.code(), asc::ErrorCode::kNumerical);
    ASC_DENSE_TEST_EQ(test, report.native_info.value_or(-1), mode.n);
    ASC_DENSE_TEST_EQ(test, report.output_validity,
    ...
    threads[worker] = std::thread([&, worker] {
      ...
      for (int repeat = 0; repeat < kRepeats; ++repeat) {
        for (const bool all_normal : {false, true}) {
          ...
          if (empty || stale) {
            const asc_block_inverse_test::Scratch<T> untouched;
...
int Run(bool hermitian) {
  ...
  std::printf(
      "block inverse concurrent groups=%d workers=%d repeats=%d "
      "native_calls=%d "
