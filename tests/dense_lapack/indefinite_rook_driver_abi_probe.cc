#if defined(ASC_ROOK_DRIVER_EMITTED_PROTOTYPES)
// Original compiler emissions are renamed only inside this probe.
#include ASC_ROOK_DRIVER_EMITTED_SSYSV_ROOK_HEADER
#include ASC_ROOK_DRIVER_EMITTED_DSYSV_ROOK_HEADER
#include ASC_ROOK_DRIVER_EMITTED_CSYSV_ROOK_HEADER
#include ASC_ROOK_DRIVER_EMITTED_ZSYSV_ROOK_HEADER
#include ASC_ROOK_DRIVER_EMITTED_CHESV_ROOK_HEADER
#include ASC_ROOK_DRIVER_EMITTED_ZHESV_ROOK_HEADER
#undef ssysv_rook_
#undef dsysv_rook_
#undef csysv_rook_
#undef zsysv_rook_
...
static_assert(
    std::is_same_v<
        typename AbiSignature<decltype(LAPACK_ssysv_rook_base)>::Type,
        typename AbiSignature<decltype(asc_probe_emitted_ssysv_rook)>::Type>);
static_assert(
    std::is_same_v<
        typename AbiSignature<decltype(LAPACK_dsysv_rook_base)>::Type,
        typename AbiSignature<decltype(asc_probe_emitted_dsysv_rook)>::Type>);
static_assert(
    std::is_same_v<
        typename AbiSignature<decltype(LAPACK_csysv_rook_base)>::Type,
        typename AbiSignature<decltype(asc_probe_emitted_csysv_rook)>::Type>);
static_assert(
    std::is_same_v<
        typename AbiSignature<decltype(LAPACK_zsysv_rook_base)>::Type,
        typename AbiSignature<decltype(asc_probe_emitted_zsysv_rook)>::Type>);
static_assert(
    std::is_same_v<
        typename AbiSignature<decltype(LAPACK_chesv_rook_base)>::Type,
        typename AbiSignature<decltype(asc_probe_emitted_chesv_rook)>::Type>);
static_assert(
    std::is_same_v<
        typename AbiSignature<decltype(LAPACK_zhesv_rook_base)>::Type,
        typename AbiSignature<decltype(asc_probe_emitted_zhesv_rook)>::Type>);
}  // namespace
#endif
namespace {
void Call([[maybe_unused]] bool hermitian, const char* uplo,
          ...
          lapack_int* info) {
  LAPACK_ssysv_rook(uplo, n, nrhs, a, lda, pivots, b, ldb, work, lwork, info);
}
void Call([[maybe_unused]] bool hermitian, const char* uplo,
          ...
          lapack_int* info) {
  LAPACK_dsysv_rook(uplo, n, nrhs, a, lda, pivots, b, ldb, work, lwork, info);
}
void Call([[maybe_unused]] bool hermitian, const char* uplo,
          const lapack_int* n, const lapack_int* nrhs, std::complex<float>* a,
          const lapack_int* lda, lapack_int* pivots, std::complex<float>* b,
          const lapack_int* ldb, std::complex<float>* work,
          const lapack_int* lwork, lapack_int* info) {
  if (hermitian) {
    LAPACK_chesv_rook(uplo, n, nrhs, a, lda, pivots, b, ldb, work, lwork, info);
  } else {
    LAPACK_csysv_rook(uplo, n, nrhs, a, lda, pivots, b, ldb, work, lwork, info);
  }
}
void Call([[maybe_unused]] bool hermitian, const char* uplo,
          const lapack_int* n, const lapack_int* nrhs, std::complex<double>* a,
          const lapack_int* lda, lapack_int* pivots, std::complex<double>* b,
          const lapack_int* ldb, std::complex<double>* work,
          const lapack_int* lwork, lapack_int* info) {
  if (hermitian) {
    LAPACK_zhesv_rook(uplo, n, nrhs, a, lda, pivots, b, ldb, work, lwork, info);
  } else {
    LAPACK_zsysv_rook(uplo, n, nrhs, a, lda, pivots, b, ldb, work, lwork, info);
  }
}
template <typename T>
...
bool Probe(bool hermitian, char triangle, int mode, int columns, int policy) {
  ...
  Call(hermitian, &uplo[1], &n[1], &nrhs[1], &a[1], &lda[1], &pivots[1], &b[1],
       &ldb[1], &work[1], &lwork[1], &info[1]);
...
int main() {
  ...
  std::printf("rook driver ABI probes=%d passed=%d\n", total, passed);
