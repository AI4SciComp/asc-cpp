...
file(COPY "${SOURCE_DIR}/examples/mixed_positive/"
  DESTINATION "${WORK_DIR}/copied mixed positive example")
file(READ "${WORK_DIR}/copied mixed positive example/CMakeLists.txt" _pt_project)
file(WRITE "${WORK_DIR}/copied mixed positive example/CMakeLists.txt"
  "string(REPLACE \"|\" \";\" ASC_CPP_LAPACK_RUNTIME_LIBRARIES \"\${TEST_RUNTIMES}\")\n"
  "${_pt_project}\n")
_run(positive-example-configure TRUE "${CMAKE_COMMAND}"
  -S "${WORK_DIR}/copied mixed positive example" -B "${WORK_DIR}/mixed positive example build"
  "-DASCCpp_DIR=${_package}" "-DCMAKE_CXX_COMPILER=${CXX_COMPILER}"
  "-DASC_CPP_LAPACK_ROOT=${LAPACK_ROOT}"
  "-DTEST_RUNTIMES=${LAPACK_RUNTIMES}"
  -DCMAKE_DISABLE_FIND_PACKAGE_CUDAToolkit=TRUE
  ...
  -DCMAKE_DISABLE_FIND_PACKAGE_BLAS=TRUE
  -DCMAKE_FIND_USE_PACKAGE_REGISTRY=FALSE)
_run(positive-example-build TRUE "${CMAKE_COMMAND}" --build
  "${WORK_DIR}/mixed positive example build" --config "${CONFIG}")
_run(positive-example-test TRUE "${CMAKE_CTEST_COMMAND}" --test-dir
...
file(COPY "${SOURCE_DIR}/examples/precision_conversion/"
  DESTINATION "${WORK_DIR}/copied precision conversion example")
file(READ "${WORK_DIR}/copied precision conversion example/CMakeLists.txt" _conversion_project)
file(WRITE "${WORK_DIR}/copied precision conversion example/CMakeLists.txt"
  "string(REPLACE \"|\" \";\" ASC_CPP_LAPACK_RUNTIME_LIBRARIES \"\${TEST_RUNTIMES}\")\n"
  "${_conversion_project}\n")
_run(conversion-example-configure TRUE "${CMAKE_COMMAND}"
  -S "${WORK_DIR}/copied precision conversion example" -B "${WORK_DIR}/precision conversion example build"
  "-DASCCpp_DIR=${_package}" "-DCMAKE_CXX_COMPILER=${CXX_COMPILER}"
  "-DASC_CPP_LAPACK_ROOT=${LAPACK_ROOT}"
  "-DTEST_RUNTIMES=${LAPACK_RUNTIMES}"
  -DCMAKE_DISABLE_FIND_PACKAGE_CUDAToolkit=TRUE
  ...
  -DCMAKE_DISABLE_FIND_PACKAGE_BLAS=TRUE
  -DCMAKE_FIND_USE_PACKAGE_REGISTRY=FALSE)
_run(conversion-example-build TRUE "${CMAKE_COMMAND}" --build
  "${WORK_DIR}/precision conversion example build" --config "${CONFIG}")
_run(conversion-example-test TRUE "${CMAKE_CTEST_COMMAND}" --test-dir
  "${WORK_DIR}/precision conversion example build" -C "${CONFIG}"
  --output-on-failure --no-tests=error)

file(COPY "${SOURCE_DIR}/examples/matrix_copy/"
  DESTINATION "${WORK_DIR}/copied matrix copy example")
file(READ "${WORK_DIR}/copied matrix copy example/CMakeLists.txt" _copy_project)
file(WRITE "${WORK_DIR}/copied matrix copy example/CMakeLists.txt"
  "string(REPLACE \"|\" \";\" ASC_CPP_LAPACK_RUNTIME_LIBRARIES \"\${TEST_RUNTIMES}\")\n"
  "${_copy_project}\n")
_run(matrix-copy-example-configure TRUE "${CMAKE_COMMAND}"
  -S "${WORK_DIR}/copied matrix copy example" -B "${WORK_DIR}/matrix copy example build"
  "-DASCCpp_DIR=${_package}" "-DCMAKE_CXX_COMPILER=${CXX_COMPILER}"
  "-DASC_CPP_LAPACK_ROOT=${LAPACK_ROOT}"
  "-DTEST_RUNTIMES=${LAPACK_RUNTIMES}"
  -DCMAKE_DISABLE_FIND_PACKAGE_CUDAToolkit=TRUE
  ...
  -DCMAKE_DISABLE_FIND_PACKAGE_BLAS=TRUE
  -DCMAKE_FIND_USE_PACKAGE_REGISTRY=FALSE)
_run(matrix-copy-example-build TRUE "${CMAKE_COMMAND}" --build
  "${WORK_DIR}/matrix copy example build" --config "${CONFIG}")
_run(matrix-copy-example-test TRUE "${CMAKE_CTEST_COMMAND}" --test-dir
...
file(COPY "${SOURCE_DIR}/examples/matrix_set/"
  DESTINATION "${WORK_DIR}/copied matrix set example")
file(READ "${WORK_DIR}/copied matrix set example/CMakeLists.txt" _set_project)
file(WRITE "${WORK_DIR}/copied matrix set example/CMakeLists.txt"
  "string(REPLACE \"|\" \";\" ASC_CPP_LAPACK_RUNTIME_LIBRARIES \"\${TEST_RUNTIMES}\")\n"
  "${_set_project}\n")
_run(matrix-set-example-configure TRUE "${CMAKE_COMMAND}"
  -S "${WORK_DIR}/copied matrix set example" -B "${WORK_DIR}/matrix set example build"
  "-DASCCpp_DIR=${_package}" "-DCMAKE_CXX_COMPILER=${CXX_COMPILER}"
  "-DASC_CPP_LAPACK_ROOT=${LAPACK_ROOT}"
  "-DTEST_RUNTIMES=${LAPACK_RUNTIMES}"
  -DCMAKE_DISABLE_FIND_PACKAGE_CUDAToolkit=TRUE
  ...
  -DCMAKE_DISABLE_FIND_PACKAGE_BLAS=TRUE
  -DCMAKE_FIND_USE_PACKAGE_REGISTRY=FALSE)
_run(matrix-set-example-build TRUE "${CMAKE_COMMAND}" --build
  "${WORK_DIR}/matrix set example build" --config "${CONFIG}")
_run(matrix-set-example-test TRUE "${CMAKE_CTEST_COMMAND}" --test-dir
  "${WORK_DIR}/matrix set example build" -C "${CONFIG}"
  --output-on-failure --no-tests=error)

file(COPY "${SOURCE_DIR}/examples/positive_tridiagonal_condition/"
  DESTINATION "${WORK_DIR}/copied positive tridiagonal condition example")
file(READ "${WORK_DIR}/copied positive tridiagonal condition example/CMakeLists.txt" _set_project)
file(WRITE "${WORK_DIR}/copied positive tridiagonal condition example/CMakeLists.txt"
  "string(REPLACE \"|\" \";\" ASC_CPP_LAPACK_RUNTIME_LIBRARIES \"\${TEST_RUNTIMES}\")\n"
  "${_set_project}\n")
_run(positive-tridiagonal-condition-example-configure TRUE "${CMAKE_COMMAND}"
  -S "${WORK_DIR}/copied positive tridiagonal condition example" -B "${WORK_DIR}/positive tridiagonal condition example build"
  "-DASCCpp_DIR=${_package}" "-DCMAKE_CXX_COMPILER=${CXX_COMPILER}"
  "-DASC_CPP_LAPACK_ROOT=${LAPACK_ROOT}"
  "-DTEST_RUNTIMES=${LAPACK_RUNTIMES}"
  -DCMAKE_DISABLE_FIND_PACKAGE_CUDAToolkit=TRUE
  ...
  -DCMAKE_DISABLE_FIND_PACKAGE_BLAS=TRUE
  -DCMAKE_FIND_USE_PACKAGE_REGISTRY=FALSE)
_run(positive-tridiagonal-condition-example-build TRUE "${CMAKE_COMMAND}" --build
  "${WORK_DIR}/positive tridiagonal condition example build" --config "${CONFIG}")
_run(positive-tridiagonal-condition-example-test TRUE "${CMAKE_CTEST_COMMAND}" --test-dir
  "${WORK_DIR}/positive tridiagonal condition example build" -C "${CONFIG}"
  --output-on-failure --no-tests=error)

file(COPY "${SOURCE_DIR}/examples/positive_tridiagonal_refinement/"
  DESTINATION "${WORK_DIR}/copied positive tridiagonal refinement example")
file(READ "${WORK_DIR}/copied positive tridiagonal refinement example/CMakeLists.txt" _ptrfs_project)
file(WRITE "${WORK_DIR}/copied positive tridiagonal refinement example/CMakeLists.txt"
  "string(REPLACE \"|\" \";\" ASC_CPP_LAPACK_RUNTIME_LIBRARIES \"\${TEST_RUNTIMES}\")\n"
  "${_ptrfs_project}\n")
_run(positive-tridiagonal-refinement-example-configure TRUE "${CMAKE_COMMAND}"
  -S "${WORK_DIR}/copied positive tridiagonal refinement example" -B "${WORK_DIR}/positive tridiagonal refinement example build"
  "-DASCCpp_DIR=${_package}" "-DCMAKE_CXX_COMPILER=${CXX_COMPILER}"
  "-DASC_CPP_LAPACK_ROOT=${LAPACK_ROOT}"
  "-DTEST_RUNTIMES=${LAPACK_RUNTIMES}"
  -DCMAKE_DISABLE_FIND_PACKAGE_CUDAToolkit=TRUE
  ...
  -DCMAKE_DISABLE_FIND_PACKAGE_BLAS=TRUE
  -DCMAKE_FIND_USE_PACKAGE_REGISTRY=FALSE)
_run(positive-tridiagonal-refinement-example-build TRUE "${CMAKE_COMMAND}" --build
  "${WORK_DIR}/positive tridiagonal refinement example build" --config "${CONFIG}")
_run(positive-tridiagonal-refinement-example-test TRUE "${CMAKE_CTEST_COMMAND}" --test-dir
...
file(COPY "${SOURCE_DIR}/examples/positive_tridiagonal_expert/"
  DESTINATION "${WORK_DIR}/copied positive tridiagonal expert example")
file(READ "${WORK_DIR}/copied positive tridiagonal expert example/CMakeLists.txt" _ptsvx_project)
file(WRITE "${WORK_DIR}/copied positive tridiagonal expert example/CMakeLists.txt"
  "string(REPLACE \"|\" \";\" ASC_CPP_LAPACK_RUNTIME_LIBRARIES \"\${TEST_RUNTIMES}\")\n"
  "${_ptsvx_project}\n")
_run(positive-tridiagonal-expert-example-configure TRUE "${CMAKE_COMMAND}"
  -S "${WORK_DIR}/copied positive tridiagonal expert example" -B "${WORK_DIR}/positive tridiagonal expert example build"
  "-DASCCpp_DIR=${_package}" "-DCMAKE_CXX_COMPILER=${CXX_COMPILER}"
  "-DASC_CPP_LAPACK_ROOT=${LAPACK_ROOT}"
  "-DTEST_RUNTIMES=${LAPACK_RUNTIMES}"
  -DCMAKE_DISABLE_FIND_PACKAGE_CUDAToolkit=TRUE
  ...
  -DCMAKE_DISABLE_FIND_PACKAGE_BLAS=TRUE
  -DCMAKE_FIND_USE_PACKAGE_REGISTRY=FALSE)
_run(positive-tridiagonal-expert-example-build TRUE "${CMAKE_COMMAND}" --build
  "${WORK_DIR}/positive tridiagonal expert example build" --config "${CONFIG}")
_run(positive-tridiagonal-expert-example-test TRUE "${CMAKE_CTEST_COMMAND}" --test-dir
  "${WORK_DIR}/positive tridiagonal expert example build" -C "${CONFIG}"
  --output-on-failure --no-tests=error)
...
file(WRITE "${WORK_DIR}/copied dmd example/CMakeLists.txt"
  "string(REPLACE \"|\" \";\" ASC_CPP_LAPACK_RUNTIME_LIBRARIES \"\${TEST_RUNTIMES}\")\n"
  "${_dmd_project}\n")
_run(dmd-example-configure TRUE "${CMAKE_COMMAND}"
  -S "${WORK_DIR}/copied dmd example" -B "${WORK_DIR}/dmd example build"
  "-DASCCpp_DIR=${_package}" "-DCMAKE_CXX_COMPILER=${CXX_COMPILER}"
  "-DASC_CPP_LAPACK_ROOT=${LAPACK_ROOT}"
  "-DTEST_RUNTIMES=${LAPACK_RUNTIMES}"
  -DCMAKE_DISABLE_FIND_PACKAGE_CUDAToolkit=TRUE
  ...
  -DCMAKE_DISABLE_FIND_PACKAGE_BLAS=TRUE
  -DCMAKE_FIND_USE_PACKAGE_REGISTRY=FALSE)
_run(dmd-example-build TRUE "${CMAKE_COMMAND}" --build
  "${WORK_DIR}/dmd example build" --config "${CONFIG}")
_run(dmd-example-test TRUE "${CMAKE_CTEST_COMMAND}" --test-dir
...
if(NOT _linkage_error AND _linkage STREQUAL "shared")
  ...
  asc_cpp_check_shared_lapack_runtime(
    "${WORK_DIR}/matrix set example build/matrix_set"
    "${LAPACK_ROOT}" "${_metadata}" "${WORK_DIR}/matrix-set-runtime-checks.log")
  asc_cpp_check_shared_lapack_runtime(
    "${WORK_DIR}/positive tridiagonal condition example build/positive_tridiagonal_condition"
    "${LAPACK_ROOT}" "${_metadata}" "${WORK_DIR}/positive-tridiagonal-condition-runtime-checks.log")
  asc_cpp_check_shared_lapack_runtime(
  ...
  asc_cpp_check_shared_lapack_runtime(
    "${WORK_DIR}/positive tridiagonal expert example build/positive_tridiagonal_expert"
    "${LAPACK_ROOT}" "${_metadata}" "${WORK_DIR}/positive-tridiagonal-expert-runtime-checks.log")
