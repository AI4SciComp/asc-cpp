...
// triangular inverse/solve routines. They are not compiler emissions.
...
template <typename T>
using Inverse = void(char*, char*, Integer*, T*, Integer*, std::size_t,
                     std::size_t);
template <typename T>
using Solve = void(char*, char*, char*, Integer*, Integer*, T*, T*, Integer*,
                   Integer*, std::size_t, std::size_t, std::size_t);
static_assert(kMatches<decltype(LAPACK_stptri_base), Inverse<float>>);
static_assert(kMatches<decltype(LAPACK_stptrs_base), Solve<float>>);
static_assert(kMatches<decltype(LAPACK_dtptri_base), Inverse<double>>);
static_assert(kMatches<decltype(LAPACK_dtptrs_base), Solve<double>>);
static_assert(
    kMatches<decltype(LAPACK_ctptri_base), Inverse<std::complex<float>>>);
static_assert(
    kMatches<decltype(LAPACK_ctptrs_base), Solve<std::complex<float>>>);
static_assert(
    kMatches<decltype(LAPACK_ztptri_base), Inverse<std::complex<double>>>);
static_assert(
    kMatches<decltype(LAPACK_ztptrs_base), Solve<std::complex<double>>>);
...
int main() {
  ...
  std::printf(
      "8 complete packed triangular signatures passed; configured "
      "INTEGER bits=%d, exact primitive types and two/three size_t CHARACTER "
      "lengths.\n",
