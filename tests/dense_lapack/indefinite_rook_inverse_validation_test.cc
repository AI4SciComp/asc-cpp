...
#include "../../src/dense/lapack/internal_indefinite_rook_inverse_counts.h"
...
using asc_rook_inverse_test::Inverse;
using asc_rook_inverse_test::Query;
...
void Counts(TestContext& test, bool hermitian) {
  ...
  const auto count = [hermitian](asc::extent_t n, asc::extent_t lda) {
    return asc::internal_indefinite_rook_inverse_counts::Inverse(
        n, lda, hermitian, kLimit);
  };
...
int Run(bool hermitian) {
  ...
  std::printf("rook inverse rejection cases=%d\n", cases);
  return test.Finish();
}
}  // namespace
