...
using asc_rk_driver_test::Driver;
using asc_rk_driver_test::Query;
...
template <typename T>
void AlterExtra(int bad, std::array<T, 16>& a, std::array<T, 16>& b,
                std::array<T, 5>& e, std::array<asc::index_t, 5>& p,
                asc::DenseBlasVectorView<T>& extra,
                asc::LapackWorkspace& workspace, asc::LapackReport& report) {
  switch (bad) {
    case 22:
      extra = asc_rk_test::OffDiagonal(e, 1);
      break;
    case 23:
      extra = base::Take(asc::DenseBlasVectorView<T>::Create(
          e.data(), 2, 2, {e.data(), sizeof(e), base::kHost}));
      break;
    case 24:
      extra = base::Take(asc::DenseBlasVectorView<T>::Create(
          e.data(), 2, 1,
      ...
      break;
    case 25:
      extra = asc_rk_test::OffDiagonal(a, 2);
      break;
    case 26:
      extra = asc_rk_test::OffDiagonal(b, 2);
      break;
    case 27:
      extra = base::Take(asc::DenseBlasVectorView<T>::Create(
          reinterpret_cast<T*>(p.data() + 1), 2, 1,
          {p.data(), sizeof(p), base::kHost}));
      break;
    case 28:
      workspace.regions[base::kScalar] = {e.data() + 1, sizeof(T), base::kHost};
      break;
    case 29:
      extra = base::Take(asc::DenseBlasVectorView<T>::Create(
          reinterpret_cast<T*>(&report), 2, 1,
...
void AlterOperands(int bad, asc::DenseBlasTriangle triangle,
                   ...
                   asc::LapackWorkspace& workspace, asc::LapackReport& report) {
  AlterExtra(bad, a, b, e, p, extra, workspace, report);
  switch (bad) {
    case 12:
...
void Rejection(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  auto extra = asc_rk_test::OffDiagonal(e, 2);
  auto plan = base::Take(base::WithoutAllocation(test, [&] {
    return Query(provider, triangle, hermitian, matrix, extra, pivots, rhs);
  }));
  ...
  AlterOperands(bad, triangle, layout, rhs_layout, a, b, e, p, matrix, rhs,
                extra, pivots, selected, workspace, report);
  ...
  const auto status = base::WithoutAllocation(test, [&] {
    return Driver(provider, selected, hermitian, matrix, extra, pivots, rhs,
  ...
  ASC_DENSE_TEST_EQ(test, e, before_e);
...
void Unread(TestContext& test, const asc::ReferenceLapackProvider& provider,
  ...
  auto* e = reinterpret_cast<T*>(static_cast<std::byte*>(memory) + 768);
  auto* p =
...
int Run(bool hermitian) {
  ...
  std::printf("RK driver rejection cases=%d\n", cases);
  return test.Finish();
}
}  // namespace
