...
void CheckCasts(asc_core_test::TestContext& context) {
  ...
  ASC_TEST_CHECK(context, !negative_to_unsigned.ok());
  ...
  ASC_TEST_CHECK(context, !positive_overflow.ok());
  ASC_TEST_EQ(context, positive_overflow.status().code(),
              asc::ErrorCode::kOverflow);

  const auto unsigned_overflow =
      asc::CheckedCast<std::int64_t>(std::numeric_limits<std::uint64_t>::max());
  ASC_TEST_CHECK(context, !unsigned_overflow.ok());
}

void CheckArithmetic(asc_core_test::TestContext& context) {
  const auto sum = asc::CheckedAdd(std::int64_t{17}, std::int64_t{-8});
  ASC_TEST_CHECK(context, sum.ok());
  ASC_TEST_EQ(context, *sum, 9);

  const auto maximum_sum = asc::CheckedAdd(
      std::numeric_limits<std::int64_t>::max(), std::int64_t{0});
  ASC_TEST_CHECK(context, maximum_sum.ok());

  const auto positive_overflow = asc::CheckedAdd(
      std::numeric_limits<std::int64_t>::max(), std::int64_t{1});
  ASC_TEST_CHECK(context, !positive_overflow.ok());
  const auto negative_overflow = asc::CheckedAdd(
      std::numeric_limits<std::int64_t>::min(), std::int64_t{-1});
  ASC_TEST_CHECK(context, !negative_overflow.ok());

  const auto unsigned_overflow = asc::CheckedAdd(
      std::numeric_limits<std::uint64_t>::max(), std::uint64_t{1});
  ASC_TEST_CHECK(context, !unsigned_overflow.ok());

  const auto product = asc::CheckedMultiply(std::int64_t{-7}, std::int64_t{-9});
  ASC_TEST_CHECK(context, product.ok());
  ASC_TEST_EQ(context, *product, 63);
  ASC_TEST_EQ(context,
              *asc::CheckedMultiply(std::int64_t{0},
                                    std::numeric_limits<std::int64_t>::min()),
              0);

  constexpr std::int64_t kHalfMaximum =
      std::numeric_limits<std::int64_t>::max() / 2;
  ASC_TEST_CHECK(context,
                 asc::CheckedMultiply(kHalfMaximum, std::int64_t{2}).ok());
  ASC_TEST_CHECK(context,
  ...
  for (int left = std::numeric_limits<std::int8_t>::min();
    ...
    for (int right = std::numeric_limits<std::int8_t>::min();
      ...
      if (sum_fits) {
        ASC_TEST_EQ(context, static_cast<int>(*checked_sum), reference_sum);
      }
      ...
      if (product_fits) {
        ASC_TEST_EQ(context, static_cast<int>(*checked_product),
                    reference_product);
      }
    }
  }
...
void CheckByteCounts(asc_core_test::TestContext& context) {
  const auto bytes = asc::CheckedByteCount(11, sizeof(std::uint32_t));
  ASC_TEST_CHECK(context, bytes.ok());
  ASC_TEST_EQ(context, *bytes, std::size_t{44});
  ASC_TEST_EQ(context, *asc::CheckedByteCount(0, 4096), std::size_t{0});

  const auto negative = asc::CheckedByteCount(-1, 1);
  ASC_TEST_CHECK(context, !negative.ok());
  ASC_TEST_EQ(context, negative.status().code(),
              asc::ErrorCode::kInvalidArgument);

  const asc::extent_t too_many = std::numeric_limits<asc::extent_t>::max();
  const auto overflow =
      asc::CheckedByteCount(too_many, std::numeric_limits<std::size_t>::max());
  ASC_TEST_CHECK(context, !overflow.ok());
...
void CheckExtents(asc_core_test::TestContext& context) {
  ...
  ASC_TEST_CHECK(context, scalar.ok());
  ASC_TEST_EQ(context, scalar->logical_size(), 1);
  ASC_TEST_CHECK(context, scalar->values().empty());
  const auto scalar_dimension = scalar->extent(0);
  ASC_TEST_CHECK(context, !scalar_dimension.ok());
  ASC_TEST_EQ(context, scalar_dimension.status().code(),
  ...
  static_assert(MixedExtents::static_extent<1>() == asc::kDynamicExtent);
  const auto mixed = MixedExtents::Create(3);
  ASC_TEST_CHECK(context, mixed.ok());
  ...
  ASC_TEST_CHECK(context, !mixed->extent(3).ok());
  ...
  const auto negative = MixedExtents::Create(-1);
  ASC_TEST_CHECK(context, !negative.ok());
  ASC_TEST_EQ(context, negative.status().code(), asc::ErrorCode::kShape);

  using OneDynamicExtent = asc::Extents<asc::kDynamicExtent>;
  const auto conversion_overflow =
      OneDynamicExtent::Create(std::numeric_limits<std::uint64_t>::max());
  ASC_TEST_CHECK(context, !conversion_overflow.ok());
  ASC_TEST_EQ(context, conversion_overflow.status().code(),
              asc::ErrorCode::kOverflow);

  using ProductExtents = asc::Extents<asc::kDynamicExtent, asc::kDynamicExtent>;
  const auto product_overflow =
      ProductExtents::Create(std::numeric_limits<asc::extent_t>::max(), 2);
  ASC_TEST_CHECK(context, !product_overflow.ok());
  ASC_TEST_EQ(context, product_overflow.status().code(),
              asc::ErrorCode::kOverflow);

  const std::array<asc::extent_t, 1> wrong_count = {5};
  const auto arity_failure =
      ProductExtents::Create(std::span<const asc::extent_t>(wrong_count));
  ASC_TEST_CHECK(context, !arity_failure.ok());
  ASC_TEST_EQ(context, arity_failure.status().code(), asc::ErrorCode::kShape);
}
