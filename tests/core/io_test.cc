...
void CheckPartialIo(asc_core_test::TestContext& context) {
  ...
  ASC_TEST_EQ(
      context,
      asc::ReadExact(empty_source, std::span<std::byte>(one_byte)).code(),
  ...
  ASC_TEST_EQ(
      context,
      asc::ReadExact(short_source, std::span<std::byte>(three_bytes)).code(),
      asc::ErrorCode::kEndOfFile);
  ...
  ASC_TEST_EQ(
      context,
      asc::ReadExact(failing_source, std::span<std::byte>(destination)).code(),
      asc::ErrorCode::kIo);
  ...
  ASC_TEST_EQ(
      context,
      asc::WriteAll(failing_sink, std::span<const std::byte>(expected)).code(),
      asc::ErrorCode::kIo);
...
void CheckLittleEndian(asc_core_test::TestContext& context) {
  ...
  const std::array<std::byte, 4> expected_float = {
      std::byte{0x00}, std::byte{0x00}, std::byte{0x80}, std::byte{0x3f}};
  ASC_TEST_CHECK(context, one_float == expected_float);
  ...
  std::array<std::byte, 3> wrong_size = {std::byte{0x5a}, std::byte{0x5a},
                                         std::byte{0x5a}};
  ASC_TEST_EQ(context,
              asc::EncodeLittleEndian(std::uint16_t{1},
                                      std::span<std::byte>(wrong_size))
                  .code(),
  ...
  ASC_TEST_EQ(context,
              asc::DecodeLittleEndian<std::uint32_t>(
...
void CheckFiles(asc_core_test::TestContext& context) {
  ...
  ASC_TEST_EQ(context, *asc::ReadTextFile(path, text.size()), text);
  ASC_TEST_EQ(context,
              asc::ReadTextFile(path, text.size() - 1U).status().code(),
              asc::ErrorCode::kOverflow);
  ASC_TEST_CHECK(context, asc::WriteTextFile(second_path, "").ok());
  ASC_TEST_CHECK(context, asc::ReadTextFile(second_path, 0)->empty());
