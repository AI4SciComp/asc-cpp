...
void CheckStatus(asc_core_test::TestContext& context) {
  ...
  const asc::Status explicit_ok = asc::Status::Ok();
  ASC_TEST_CHECK(context, explicit_ok.ok());
...
void CheckResult(asc_core_test::TestContext& context) {
  ...
  *value = 41;
  ...
  const asc::Status failure_status(asc::ErrorCode::kUnavailable,
                                   "backend unavailable");
  const asc::Result<int> failure(failure_status);
