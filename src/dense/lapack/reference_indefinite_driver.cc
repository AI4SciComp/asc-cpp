...
std::string_view Name(bool hermitian) {
  if constexpr (std::is_same_v<T, float>) {
    ...
    return hermitian ? "chesv" : "csysv";
  } else {
    return hermitian ? "zhesv" : "zsysv";
  }
}
...
void Call([[maybe_unused]] bool hermitian, char triangle, lapack_int n,
          ...
          lapack_int& info) {
  LAPACK_ssysv(&triangle, &n, &nrhs, a, &lda, pivots, b, &ldb, work, &lwork,
               &info);
}
...
void Call([[maybe_unused]] bool hermitian, char triangle, lapack_int n,
          ...
          lapack_int& info) {
  LAPACK_dsysv(&triangle, &n, &nrhs, a, &lda, pivots, b, &ldb, work, &lwork,
               &info);
}
...
void Call(bool hermitian, char triangle, lapack_int n, lapack_int nrhs,
  ...
  if (hermitian) {
    LAPACK_chesv(&triangle, &n, &nrhs, a, &lda, pivots, b, &ldb, work, &lwork,
                 &info);
  } else {
    LAPACK_csysv(&triangle, &n, &nrhs, a, &lda, pivots, b, &ldb, work, &lwork,
                 &info);
  }
}
...
void Call(bool hermitian, char triangle, lapack_int n, lapack_int nrhs,
          ...
          lapack_int lwork, lapack_int& info) {
  if (hermitian) {
    LAPACK_zhesv(&triangle, &n, &nrhs, a, &lda, pivots, b, &ldb, work, &lwork,
                 &info);
  } else {
    LAPACK_zsysv(&triangle, &n, &nrhs, a, &lda, pivots, b, &ldb, work, &lwork,
                 &info);
  }
}
...
Status Publish(DenseBlasMatrixView<T> matrix,
  ...
  if (!bk::Paired(native, matrix.rows(), triangle).ok()) {
    return bk::Defect(info, report);
  }
...
Result<LapackWorkspacePlan> QuerySysvWorkspace(
...
Status Sysv(const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
...
Result<LapackWorkspacePlan> QueryHesvWorkspace(
    const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
    DenseBlasMatrixView<std::complex<float>> matrix,
    DenseBlasVectorView<index_t> pivots,
    DenseBlasMatrixView<std::complex<float>> rhs) {
...
Status Hesv(const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
            DenseBlasMatrixView<std::complex<float>> matrix,
            DenseBlasVectorView<index_t> pivots,
            DenseBlasMatrixView<std::complex<float>> rhs,
            const LapackWorkspacePlan& plan, const LapackWorkspace& workspace,
