...
#include "internal_indefinite_inverse_counts.h"
...
std::string_view Name(bool hermitian) {
  if constexpr (std::is_same_v<T, float>) {
    ...
    return hermitian ? "chetri" : "csytri";
  } else {
    return hermitian ? "zhetri" : "zsytri";
  }
}
...
void Call(bool hermitian, char triangle, lapack_int n, T* a, lapack_int lda,
  ...
  if constexpr (std::is_same_v<T, float>) {
    LAPACK_ssytri(&triangle, &n, a, &lda, pivots, work, &info);
  } else if constexpr (std::is_same_v<T, double>) {
    LAPACK_dsytri(&triangle, &n, a, &lda, pivots, work, &info);
  } else if constexpr (std::is_same_v<T, std::complex<float>>) {
    if (hermitian) {
      LAPACK_chetri(&triangle, &n, a, &lda, pivots, work, &info);
    } else {
      LAPACK_csytri(&triangle, &n, a, &lda, pivots, work, &info);
    }
  } else {
    if (hermitian) {
      LAPACK_zhetri(&triangle, &n, a, &lda, pivots, work, &info);
    } else {
      LAPACK_zsytri(&triangle, &n, a, &lda, pivots, work, &info);
    }
  }
}
...
Result<LapackWorkspacePlan> Query(const ReferenceLapackProvider& provider,
  ...
  for (const Status& status :
       {bk::Matrix(provider, factors),
        ...
        internal_indefinite_inverse_counts::Inverse(
  ...
  if (factors.rows() != 0) {
    ...
    // CSYTRI/ZSYTRI document 2*N even though this pinned implementation
    // addresses only the first N-1 entries. Honor the documented buffer.
    const extent_t multiplier = DenseBlasComplex<T> && !hermitian ? 2 : 1;
    if (n > std::numeric_limits<extent_t>::max() / multiplier) {
      return Status(ErrorCode::kOverflow);
    }
    const extent_t work_entries = multiplier * n;
    plan.regions[bk::kScalar] = {work_entries, work_entries, sizeof(T),
                                 alignof(T)};
...
Result<LapackWorkspacePlan> QuerySytriWorkspace(
...
Status Sytri(const ReferenceLapackProvider& provider,
...
Result<LapackWorkspacePlan> QueryHetriWorkspace(
    const ReferenceLapackProvider& provider, DenseBlasTriangle triangle,
    DenseBlasMatrixView<std::complex<float>> factors,
    RawLapackPivotView pivots) {
  return Query(provider, triangle, factors, pivots, true);
}
Status Hetri(const ReferenceLapackProvider& provider,
             DenseBlasTriangle triangle,
             DenseBlasMatrixView<std::complex<float>> factors,
             RawLapackPivotView pivots, const LapackWorkspacePlan& plan,
             const LapackWorkspace& workspace, LapackReport& report) {
